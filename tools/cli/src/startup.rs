...
impl StartupOptimizer {
    ...
    pub async fn initialize(&self) -> Result<Arc<CliResources>, StartupError> {
        ...
        let resources = CLI_RESOURCES.get_or_init(|| async {
            self.initialize_resources().await
        }).await;

        let init_duration = init_start.elapsed();
        if init_duration > Duration::from_millis(100) {
            warn!("CLI initialization took {:.2}ms", init_duration.as_secs_f64() * 1000.0);
        } else {
            debug!("CLI initialized in {:.2}ms", init_duration.as_secs_f64() * 1000.0);
        }
    ...
    async fn initialize_resources(&self) -> CliResources {
        ...
        debug!("Config loaded in {:.2}ms", config_start.elapsed().as_secs_f64() * 1000.0);

        // Initialize performance tracker
        let perf_start = Instant::now();
        let performance_tracker = Arc::new(PerformanceTracker::new());
        
        debug!("Performance tracker initialized in {:.2}ms", 
    ...
    pub fn setup_logging(&self) -> Result<(), StartupError> {
        ...
        debug!("Logging setup in {:.2}ms", log_start.elapsed().as_secs_f64() * 1000.0);
        Ok(())
    }
    ...
    pub async fn pre_warm(&self) -> Result<(), StartupError> {
        ...
        debug!("Pre-warming completed in {:.2}ms", warm_start.elapsed().as_secs_f64() * 1000.0);
        Ok(())
    }
...
impl CommandExecutor {
    ...
    pub async fn execute_cached<F, R>(&self, 
        command_key: &str, 
        ttl: Duration,
        executor: F
    ...
    fn cache_result<R>(&self, key: &str, result: &R, ttl: Duration) -> Result<(), Box<dyn std::error::Error + Send + Sync>>
    where
        R: serde::Serialize,
    {
...
mod tests {
    ...
    async fn test_command_executor() {
        ...
        let result = executor.execute_cached(
            ...
            async { Ok::<String, Box<dyn std::error::Error + Send + Sync>>("test_result".to_string()) }
        ).await.unwrap();
