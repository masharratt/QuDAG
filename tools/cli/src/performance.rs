...
impl PerformanceTracker {
    ...
    pub async fn record_command_completion(&self, command: &str, duration: Duration, success: bool) {
        // Record command timing
        let mut command_times = self.command_times.lock().await;
        command_times.entry(command.to_string())
        ...
        if total_completed > 0 {
            let total_duration = command_times.values()
                .flatten()
                .sum::<Duration>();
    ...
    pub async fn generate_report(&self) -> String {
        ...
        for (command, duration) in &metrics.command_execution_times {
            report.push_str(&format!("  {}: {:.2}ms\n", 
                command, duration.as_secs_f64() * 1000.0));
        }
        ...
        report.push_str(&format!("  Average Duration: {:.2}ms\n", 
            metrics.async_task_metrics.average_task_duration.as_secs_f64() * 1000.0));
...
impl CommandTracker {
    ...
    pub async fn complete(self, success: bool) {
        ...
        if duration > Duration::from_millis(100) {
            warn!("Command '{}' took {:.2}ms", self.command, duration.as_secs_f64() * 1000.0);
        } else {
            info!("Command '{}' completed in {:.2}ms", self.command, duration.as_secs_f64() * 1000.0);
        }
    }
...
impl AsyncOptimizer {
    ...
    where
        F: Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T, Box<dyn std::error::Error + Send + Sync>>> + Send>> + Send + Sync,
    {
