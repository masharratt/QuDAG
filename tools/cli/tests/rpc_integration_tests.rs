use anyhow::Result;
use qudag_cli::{CommandRouter, rpc::{RpcClient, is_node_running}};
...
async fn test_peer_management_integration() -> Result<()> {
    ...
    // Test adding a peer
    let result = router.handle_peer_add("127.0.0.1:8001".to_string(), Some(8900), None).await;
    assert!(result.is_ok());
...
async fn test_rpc_client_connectivity() -> Result<()> {
    ...
    let client = RpcClient::new_tcp("127.0.0.1".to_string(), 9999)
...
async fn test_peer_command_validation() -> Result<()> {
    ...
    let result = router.handle_peer_add("invalid_address".to_string(), Some(8901), None).await;
    assert!(result.is_err());
    
    // Test valid address format (even though connection will fail)
    let result = router.handle_peer_add("127.0.0.1:8001".to_string(), Some(8901), None).await;
    // This should fail due to connection error, not validation error
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Failed to add peer"));
...
async fn test_rpc_timeout_handling() -> Result<()> {
    let client = RpcClient::new_tcp("127.0.0.1".to_string(), 9997)
...
async fn test_concurrent_rpc_requests() -> Result<()> {
    ...
    let client = RpcClient::new_tcp("127.0.0.1".to_string(), 9996)
        .with_timeout(Duration::from_millis(100));
    
    // Launch multiple concurrent requests
    let client_clone = RpcClient::new_tcp("127.0.0.1".to_string(), 9996)
        .with_timeout(Duration::from_millis(100));
    
    let (result1, result2) = tokio::join!(
        client.list_peers(),
        client_clone.get_network_stats()
    );
