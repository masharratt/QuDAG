...
mod bandwidth_tests {
    ...
    fn test_bandwidth_monitoring_low_bandwidth_warning() {
        ...
        assert!(warning.as_ref().unwrap().contains("Upload: 0.50 Mbps (minimum: 1.00 Mbps)"));
    }
...
mod latency_tests {
    ...
    fn test_latency_monitoring_high_latency_warning() {
        ...
        assert!(warning.as_ref().unwrap().contains("Average: 250.00 ms (threshold: 100.00 ms)"));
    }
...
fn format_network_stats(stats: &NetworkStats) -> String {
    ...
    output.push_str(&format!("Bandwidth:            {:.2} Mbps\n", stats.bandwidth_mbps));
    output.push_str(&format!("Packet Loss Rate:     {:.2}%\n", stats.packet_loss_rate));
...
fn format_network_test_results(result: &NetworkTestResult) -> String {
    ...
    let check_mark = if result.configuration_valid { "✓" } else { "✗" };
    let status = if result.configuration_valid { "Valid" } else { "Failed" };
    output.push_str(&format!("{} Configuration:        {}\n", check_mark, status));
    
    let check_mark = if result.port_binding_success { "✓" } else { "✗" };
    let status = if result.port_binding_success { "Success" } else { "Failed" };
    output.push_str(&format!("{} Port Binding:         {}\n", check_mark, status));
    
    let check_mark = if result.peer_discovery_working { "✓" } else { "✗" };
    let status = if result.peer_discovery_working { "Working" } else { "Failed" };
    output.push_str(&format!("{} Peer Discovery:       {}\n", check_mark, status));
    
    let check_mark = if result.message_routing_working { "✓" } else { "✗" };
    let status = if result.message_routing_working { "Working" } else { "Failed" };
    output.push_str(&format!("{} Message Routing:      {}\n", check_mark, status));
    
    let check_mark = if result.latency_test_passed { "✓" } else { "✗" };
    let status = if result.latency_test_passed { "Passed" } else { "Failed" };
    output.push_str(&format!("{} Latency Test:         {}\n", check_mark, status));
    
    let check_mark = if result.bandwidth_test_passed { "✓" } else { "✗" };
    let status = if result.bandwidth_test_passed { "Passed" } else { "Failed" };
    output.push_str(&format!("{} Bandwidth Test:       {}\n", check_mark, status));
...
fn format_bandwidth_metrics(metrics: &BandwidthMetrics) -> String {
    ...
    output.push_str(&format!("Upload Speed:         {:.2} Mbps\n", metrics.upload_mbps));
    output.push_str(&format!("Download Speed:       {:.2} Mbps\n", metrics.download_mbps));
    output.push_str(&format!("Peak Upload:          {:.2} Mbps\n", metrics.peak_upload_mbps));
    output.push_str(&format!("Peak Download:        {:.2} Mbps\n", metrics.peak_download_mbps));
    output.push_str(&format!("Average Upload:       {:.2} Mbps\n", metrics.average_upload_mbps));
    output.push_str(&format!("Average Download:     {:.2} Mbps\n", metrics.average_download_mbps));
...
fn execute_bandwidth_monitor_continuous(duration: Duration) -> Result<Vec<BandwidthMetrics>, String> {
...
fn format_latency_metrics(metrics: &LatencyMetrics) -> String {
    ...
    output.push_str(&format!("Average Latency:      {:.2} ms\n", metrics.average_ms));
    output.push_str(&format!("Median Latency:       {:.2} ms\n", metrics.median_ms));
...
fn format_connectivity_results(result: &ConnectivityCheckResult) -> String {
    ...
    output.push_str(&format!("Reachable Peers:      {}\n", result.reachable_peers.len()));
    output.push_str(&format!("Unreachable Peers:    {}\n", result.unreachable_peers.len()));
