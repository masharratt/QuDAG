...
async fn test_cli_peer_management() {
    ...
    // Test peer addition
    node.add_peer("192.168.1.50:8080".to_string()).await
    ...
    node.remove_peer(&peer_id).await.expect("Failed to remove peer");
...
async fn test_cli_network_stats() {
    ...
    node.add_peer("10.0.0.1:8080".to_string()).await.expect("Failed to add peer");
    node.add_peer("10.0.0.2:8080".to_string()).await.expect("Failed to add peer");
...
async fn test_cli_status_command() {
    ...
    // Add some test data
    node.add_peer("peer1.example.com:8080".to_string()).await.expect("Failed to add peer");
    node.simulate_activity().await;
    ...
    println!("  Pending Transactions: {}", status.dag_stats.pending_transactions);
    println!();
    
    println!("Memory Usage:");
    println!("  Current: {} bytes", status.memory_usage.current_usage);
    println!("  Peak: {} bytes", status.memory_usage.peak_usage);
    println!("  Total Allocated: {} bytes", status.memory_usage.total_allocated);
...
async fn test_cli_error_scenarios() {
    let scenario = TestScenarioBuilder::new()
        ...
        .with_global_behavior("problematic_operation".to_string(), MockBehavior {
...
async fn test_cli_concurrent_operations() {
    ...
    println!("Successfully processed {} concurrent requests", history.len());
}
...
async fn test_cli_complex_scenario() {
    ...
    println!("  Total Connections: {}", stats.total_connections);
    println!("  Active Connections: {}", stats.active_connections);
    println!("  Messages Exchanged: {}", stats.messages_sent + stats.messages_received);
    println!("  Data Transferred: {} KB", (stats.bytes_sent + stats.bytes_received) / 1024);
    println!("  Average Latency: {:.2} ms", stats.average_latency);
    ...
    for worker in ["worker-1", "worker-2", "monitor"] {
        ...
        assert_eq!(peer_count, 1, "{} should have 1 peer connection to gateway", worker);
    }
    ...
    for (node_id, node) in &scenario.nodes {
        ...
        assert!(response.result.is_some(), "Status request failed for {}", node_id);
        ...
        assert!(response.result.is_some(), "Heartbeat failed for {}", node_id);
