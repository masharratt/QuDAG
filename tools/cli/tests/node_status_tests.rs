...
pub use qudag_cli::{execute_status_command, StatusArgs, OutputFormat, NodeStatusResponse, NodeState, PeerStatusInfo as PeerStatus, NetworkStatistics, DagStatistics, MemoryUsage};
...
mod error_handling_tests {
    ...
    async fn test_connection_refused_error() {
        ...
        mock.expect_get_node_status()
            .times(1)
            .returning(|| Err(anyhow::anyhow!("Connection refused: No node running on port 8000")));
    ...
    async fn test_invalid_response_format_error() {
        ...
        mock.expect_get_node_status()
            .times(1)
            .returning(|| Err(anyhow::anyhow!("Invalid response format: expected JSON object")));
    ...
    async fn test_rpc_server_internal_error() {
        ...
        mock.expect_get_node_status()
            .times(1)
            .returning(|| Err(anyhow::anyhow!("RPC server error 500: Internal server error")));
...
mod property_tests {
    ...
    fn test_peer_count_consistency() {
        ...
        for count in peer_counts {
            let peers: Vec<PeerStatus> = (0..count).map(|i| {
                PeerStatus {
            ...
            assert_eq!(status.connected_peers.len(), status.network_stats.active_connections);
        }
    }
