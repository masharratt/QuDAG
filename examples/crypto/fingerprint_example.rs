...
use qudag_crypto::{Fingerprint, FingerprintError, MlDsaPublicKey};
use rand::thread_rng;
...
fn basic_fingerprint_example() -> Result<(), FingerprintError> {
    ...
    println!("   Text data fingerprint: {} (size: {} bytes)", 
             hex::encode(&text_fingerprint.data()[..8]), text_fingerprint.data().len());
    println!("   Binary data fingerprint: {} (size: {} bytes)", 
             hex::encode(&binary_fingerprint.data()[..8]), binary_fingerprint.data().len());
    println!("   Empty data fingerprint: {} (size: {} bytes)", 
             hex::encode(&empty_fingerprint.data()[..8]), empty_fingerprint.data().len());
    println!("   Large data fingerprint: {} (size: {} bytes)", 
             hex::encode(&large_fingerprint.data()[..8]), large_fingerprint.data().len());
...
fn deduplication_example() -> Result<(), FingerprintError> {
    ...
    for (filename, content) in files {
        ...
        if storage.contains_key(&fp_data) {
            println!("   {} is a duplicate (fingerprint already exists)", filename);
            duplicate_count += 1;
        } else {
            storage.insert(fp_data.clone(), content.to_vec());
            fingerprint_index.insert(fp_data, public_key);
            println!("   {} stored with fingerprint: {}", filename, hex::encode(&fingerprint.data()[..8]));
            stored_count += 1;
        }
    }
    ...
    println!("   - Storage saved: {}%", (duplicate_count * 100) / files_len);
...
fn performance_comparison() -> Result<(), FingerprintError> {
    ...
    for (label, size) in data_sizes {
        ...
        println!("     - Throughput: {:.2} MB/s", size as f64 / generation_time.as_secs_f64() / 1_048_576.0);
    }
...
fn batch_fingerprinting_example() -> Result<(), FingerprintError> {
    ...
    let messages: Vec<Vec<u8>> = (0..100)
        .map(|i| format!("Message {}: Lorem ipsum dolor sit amet, consectetur adipiscing elit.", i).into_bytes())
        .collect();
    ...
    println!("   Average per fingerprint: {:?}", batch_time / messages.len() as u32);
    println!("   Batch verification completed in {:?}", verify_time);
    println!("   Average per verification: {:?}", verify_time / messages.len() as u32);
    
    // Check uniqueness
    let unique_fingerprints: std::collections::HashSet<_> = 
        fingerprints.iter().map(|fp| fp.data().to_vec()).collect();
    
    println!("   All {} fingerprints are unique: {}", 
             messages.len(), 
...
fn custom_data_types_example() -> Result<(), FingerprintError> {
    ...
    impl Transaction {
        ...
        fn fingerprint(&self, rng: &mut (impl rand::RngCore + rand::CryptoRng)) -> Result<(Fingerprint, MlDsaPublicKey), FingerprintError> {
    ...
    println!("   Transaction fingerprint: {}", hex::encode(&tx_fingerprint.data()[..16]));
    ...
    println!("   Block fingerprint: {}", hex::encode(&block_fingerprint.data()[..16]));
