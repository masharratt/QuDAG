...
use qudag_crypto::{MlKem768, KEMError, SecretKey, Ciphertext, SharedSecret};
...
fn encapsulation_decapsulation() -> Result<(), KEMError> {
    ...
    println!("   Shared secret size: {} bytes", bob_shared_secret.as_bytes().len());
...
fn error_handling_examples() -> Result<(), KEMError> {
    ...
    match attempt_decapsulation(&secret_key) {
        Ok(secret) => println!("   Decapsulation succeeded, secret size: {}", secret.as_bytes().len()),
...
fn performance_measurement() -> Result<(), KEMError> {
    ...
    println!("   Key generation: {} operations in {:?}", NUM_ITERATIONS, keygen_duration);
    ...
    println!("   Encapsulation: {} operations in {:?}", NUM_ITERATIONS, encap_duration);
    println!("   Average per encap: {:?}", encap_duration / NUM_ITERATIONS as u32);
    ...
    println!("   Decapsulation: {} operations in {:?}", NUM_ITERATIONS, decap_duration);
    println!("   Average per decap: {:?}", decap_duration / NUM_ITERATIONS as u32);
...
fn serialization_examples() -> Result<(), KEMError> {
    ...
    let reconstructed_public_key = qudag_crypto::PublicKey::from_bytes(public_key_bytes)?;
