...
mod ml_dsa_tests {
    ...
    fn test_ml_dsa_key_generation() {
        ...
        assert!(public_key.len() > 1000, "ML-DSA public key should be substantial");
        
        // Check secret key
        let secret_key = keypair.get_secret_key();
        assert!(!secret_key.is_empty(), "Secret key should not be empty");
        assert!(secret_key.len() > public_key.len(), "Secret key should be larger than public key");
    }
    ...
    fn test_ml_dsa_signing() {
        ...
        for message in messages {
            ...
            assert!(signature.len() > 1000, "ML-DSA signature should be substantial");
        }
    }
    ...
    fn test_ml_dsa_signature_verification() {
        ...
        assert!(verified_wrong, "Mock currently returns true for all valid inputs");
    }
}
...
mod ml_kem_tests {
    ...
    fn test_ml_kem_key_generation() {
        let kem = WasmMlKem768::new();
        let keypair_js = kem.generate_key_pair().expect("Should generate KEM keypair");
    ...
    fn test_ml_kem_encapsulation() {
        ...
        assert!(js_sys::Reflect::has(&obj, &"shared_secret".into()).expect("Should have shared_secret"));
    }
...
mod blake3_tests {
    ...
    fn test_blake3_hex_encoding() {
        ...
        assert!(hex_hash.chars().all(|c| c.is_ascii_hexdigit()), "Should be valid hex");
...
mod fingerprint_tests {
    ...
    fn test_fingerprint_generation() {
        ...
        let fingerprint_js = WasmFingerprint::generate(data, &keypair_bytes)
            .expect("Should generate fingerprint");
        
        assert!(fingerprint_js.is_object(), "Should return JS object");
        
        let obj = fingerprint_js.dyn_into::<Object>().expect("Should be object");
...
mod performance_tests {
    ...
    fn test_key_generation_performance() {
        ...
        assert!(duration < 5000.0, "Key generation should complete within 5 seconds");
    ...
    fn test_hashing_performance() {
        ...
        for size in data_sizes {
            ...
            web_sys::console::log_1(&format!("BLAKE3 hashing {}KB took: {:.2}ms", size / 1024, duration).into());
...
mod integration_tests {
    ...
    fn test_cross_compatibility() {
        ...
        let keypair = WasmMlDsaKeyPair::new().expect("Should generate keypair");
        let sig1 = keypair.sign(&typed_data).expect("Should sign typed array data");
...
mod error_tests {
    ...
    fn test_invalid_hex_decoding() {
        ...
        js_sys::Reflect::set(&invalid_fingerprint, &"signature".into(), &"also invalid".into()).unwrap();
        js_sys::Reflect::set(&invalid_fingerprint, &"public_key".into(), &"not hex".into()).unwrap();
    ...
    fn test_empty_data_handling() {
        ...
        assert!(!empty_sig.is_empty(), "Should produce signature for empty message");
