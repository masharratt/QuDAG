...
impl VotingRecord {
    ...
    pub fn record_vote(&mut self, vertex_id: VertexId, voter_id: VertexId, vote: bool) -> Result<(), ConsensusError> {
            ...
            if let Some(&previous_vote) = vertex_votes.get(&voter_id) {
                if previous_vote != vote {
                    ...
                    return Err(ConsensusError::ByzantineBehavior(
                        format!("Voter {:?} changed vote for vertex {:?}", voter_id, vertex_id)
                    ));
                }
            }
        }
        
        self.votes.entry(vertex_id).or_insert_with(HashMap::new).insert(voter_id, vote);
        Ok(())
    }
...
impl QRAvalanche {
    ...
    pub fn process_vertex(&mut self, vertex_id: VertexId) -> Result<ConsensusStatus, ConsensusError> {
        // Record start time
        self.vertex_start_times.insert(vertex_id.clone(), Instant::now());
    ...
    pub fn record_vote(&mut self, vertex_id: VertexId, voter_id: VertexId, vote: bool) -> Result<(), ConsensusError> {
        // Record the vote
        self.voting_record.record_vote(vertex_id.clone(), voter_id.clone(), vote)?;
        ...
        if let Some(confidence) = self.confidence.get_mut(&vertex_id) {
            ...
            if confidence.value >= self.config.beta {
                ...
                self.vertices.insert(vertex_id.clone(), ConsensusStatus::Rejected);
    ...
    fn finalize_vertex(&mut self, vertex_id: VertexId) -> Result<(), ConsensusError> {
        ...
        self.vertices.insert(vertex_id.clone(), ConsensusStatus::Final);
    ...
    fn detect_fork_conflicts(&self) -> Vec<Vec<VertexId>> {
        ...
        let pending_vertices: Vec<_> = self.vertices
            .iter()
        ...
        for vertex_id in &pending_vertices {
            ...
            for other_vertex_id in &pending_vertices {
                if other_vertex_id != vertex_id && !visited.contains(other_vertex_id) {
                    if self.vertices_conflict(vertex_id, other_vertex_id) {
    ...
    fn resolve_conflict_set(&self, conflict_set: &[VertexId]) -> Result<VertexId, ConsensusError> {
        ...
        let mut best_confidence = self.confidence.get(best_vertex).map(|c| c.value).unwrap_or(0.0);
        
        for vertex_id in &conflict_set[1..] {
            let confidence = self.confidence.get(vertex_id).map(|c| c.value).unwrap_or(0.0);
    ...
    pub fn advanced_fork_resolution(&mut self, vertex_id: &VertexId) -> Result<ConsensusStatus, ConsensusError> {
    ...
    pub async fn query_sample(&mut self, vertex_id: &VertexId) -> Result<(usize, usize), ConsensusError> {
        ...
        for participant in sample_participants.iter().take(sample_size) {
            ...
            // Record the vote
            if let Err(_e) = self.voting_record.record_vote(vertex_id.clone(), (*participant).clone(), vote) {
    ...
    fn simulate_participant_vote(&self, vertex_id: &VertexId, participant_id: &VertexId) -> bool {
        ...
        for (i, &v_byte) in vertex_bytes.iter().enumerate() {
            if i < participant_bytes.len() {
                hash_value = hash_value.wrapping_add((v_byte as u64).wrapping_mul(participant_bytes[i] as u64));
            }
        }
    ...
    pub async fn run_consensus_round(&mut self, vertex_id: &VertexId) -> Result<ConsensusStatus, ConsensusError> {
    ...
    pub async fn run_fast_consensus_round(&mut self, vertex_id: &VertexId) -> Result<ConsensusStatus, ConsensusError> {
        let start_time = Instant::now();
        ...
        while start_time.elapsed() < target_finality_time && round < self.config.max_rounds {
            ...
            if round_confidence >= self.config.alpha * 0.95 { // Slightly lower threshold for speed
                consecutive_strong_rounds += 1;
                ...
                if current_confidence <= (1.0 - self.config.beta * 0.9) {
                    self.vertices.insert(vertex_id.clone(), ConsensusStatus::Rejected);
                    self.tips.remove(vertex_id);
                    return Ok(ConsensusStatus::Rejected);
                }
        ...
        for (vertex_id, vertex_votes) in &self.voting_record.votes {
            for (voter_id, &_vote) in vertex_votes {
                ...
                if self.has_conflicting_vote_pattern(voter_id, vertex_id) {
                    if !self.voting_record.byzantine_voters.contains(voter_id) {
    ...
    fn has_conflicting_vote_pattern(&self, voter_id: &VertexId, _vertex_id: &VertexId) -> bool {
        ...
        let voter_votes: Vec<_> = self.voting_record.votes
            .values()
