...
mod tests {
    ...
    fn test_total_order_consistency() {
        ...
        for i in 1..vertices.len() {
            ...
            assert!(parent_pos < child_pos, "Parent {} should come before child {}", vertices[i-1], vertices[i]);
        }
    }
    ...
    fn test_consensus_status_transitions() {
        ...
        assert_eq!(consensus.vertices.get(&vertex_id), Some(&ConsensusStatus::Accepted));
        assert!(consensus.tips.contains(&vertex_id));
    }
    ...
    fn test_dag_invariants() {
        ...
        for (id, parents) in vertices {
            ...
            assert!(dag.add_vertex(vertex).is_ok(), "Failed to add vertex {}", id);
        }
        
        // Verify all vertices achieved consensus
        for (id, _) in [("genesis", vec![]), ("a1", vec!["genesis"]), ("a2", vec!["genesis"]), ("b1", vec!["a1"]), ("b2", vec!["a2"]), ("merge", vec!["b1", "b2"])].iter() {
            assert_eq!(dag.get_confidence(id), Some(ConsensusStatus::Final), "Vertex {} should have final status", id);
        }
