...
use qudag_dag::{QRAvalanche, QRAvalancheConfig, VertexId, ConsensusStatus};
            ...
            |b, &vote_count| {
                b.iter(|| {
                    ...
                    for i in 0..*vote_count {
                        ...
                        black_box(consensus.record_vote(vertex_id.clone(), voter_id, vote).unwrap());
                    }
                });
            },
        );
            ...
            |b, &byzantine_count| {
                b.iter(|| {
                    ...
                    for i in 0..*byzantine_count {
                        ...
                        consensus.record_vote(vertex_id.clone(), voter_id.clone(), true).unwrap();
...
fn bench_config_variations(c: &mut Criterion) {
    ...
    let configs = vec![
        ("conservative", QRAvalancheConfig {
            beta: 0.9,
            alpha: 0.7,
