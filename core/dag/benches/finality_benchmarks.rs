        ...
        group.bench_with_input(
            ...
            |b, &node_count| {
                b.iter_custom(|iters| {
                    ...
                    println!("Node count: {}, 99th percentile latency: {}ms, Max latency: {}ms", 
                             node_count, p99_latency.as_millis(), max_latency.as_millis());
                    
                    // Verify we meet the sub-second finality requirement
                    if p99_latency > Duration::from_millis(1000) {
                        println!("WARNING: 99th percentile latency {}ms exceeds 1000ms target", 
            ...
            |b, &round_count| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("Round count: {}, Total latency: {}ms, Avg per round: {}ms", 
                                 round_count, total_latency.as_millis(), avg_latency.as_millis());
            ...
            |b, &byzantine_ratio| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("Byzantine ratio: {:.1}%, Latency: {}ms, Success: {}", 
                                 byzantine_ratio * 100.0, latency.as_millis(), result.is_ok());
            ...
            |b, &tx_count| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("Transaction count: {}, TPS: {:.2}, Latency: {}ms", 
                                 tx_count, tps, latency.as_millis());
            ...
            |b, &dag_size| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("DAG size: {}, Vertices/sec: {:.2}, Latency: {}ms", 
                                 dag_size, vertices_per_sec, latency.as_millis());
        ...
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                if estimated_memory > 100 * 1024 * 1024 {
                    println!("WARNING: Memory usage {} exceeds 100MB target", 
