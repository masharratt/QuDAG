...
use qudag_dag::{
    QRAvalanche, ConsensusStatus, ConsensusError, 
    Vertex, VertexId, DagMessage
};
...
async fn test_byzantine_fault_tolerance() {
    ...
    assert!(byzantine_count < config.node_count / 3, 
...
async fn test_sub_second_finality() {
    ...
    assert!(finality_time.as_millis() < config.target_finality_ms as u128,
        "Finality took {}ms, exceeding target of {}ms",
        finality_time.as_millis(), config.target_finality_ms);
}
...
async fn test_network_partition_resistance() {
    ...
    for i in 0..config.node_count / 2 {
        ...
        let vertex_b = create_test_vertex(&format!("partition_b_{}", i), vec![], vec![(i + 100) as u8]);
...
mod benchmark_tests {
    ...
    async fn benchmark_vertex_processing_throughput() {
        ...
        assert!(throughput >= 10000.0,
            "Throughput {} vertices/sec below target of 10,000/sec", throughput);
    }
    ...
    async fn benchmark_finality_latency() {
        ...
        assert!(avg_latency.as_millis() < 1000,
            "Average latency {}ms exceeds 1000ms target", avg_latency.as_millis());
    }
}