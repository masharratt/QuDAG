...
use qudag_dag::{
    QRAvalanche, QRAvalancheConfig, VertexId, ConsensusStatus, 
    Confidence, ConsensusMetrics
};
...
async fn test_complete_qr_avalanche_consensus() {
    ...
    for i in 0..vertex_count {
        ...
        let final_status = consensus.run_fast_consensus_round(&vertex_id).await.unwrap();
        ...
        assert!(matches!(final_status, ConsensusStatus::Final | ConsensusStatus::Accepted),
            "Vertex {} failed to achieve consensus: {:?}", i, final_status);
        ...
        if let Some(confidence) = consensus.get_confidence(&vertex_id) {
            assert!(confidence.value >= 0.7, 
                "Vertex {} has low confidence: {}", i, confidence.value);
        }
    ...
    assert!(total_time < Duration::from_secs(10), 
        ...
        total_time, vertex_count);
    
    // Check metrics
    let metrics = consensus.get_metrics();
    assert_eq!(metrics.total_vertices_processed, vertex_count);
    assert!(metrics.current_throughput > 1.0, 
        "Throughput {} vertices/sec too low", metrics.current_throughput);
    
    // Verify at least some vertices reached finality
    assert!(!finalized_vertices.is_empty(), 
    ...
    // Final verification
    assert!(consensus.tips.len() <= vertex_count, 
        "Too many tips remaining: {}", consensus.tips.len());
}
...
async fn test_high_load_consensus() {
    ...
    assert!(throughput >= 50.0, 
        "Throughput {} vertices/sec below target of 50/sec", throughput);
...
async fn test_adverse_conditions_consensus() {
    ...
    assert!(consensus_achieved, 
        "Failed to achieve consensus under adverse conditions after {} attempts", attempts);
    
    // Should still be reasonably fast even under adverse conditions
    assert!(total_time < Duration::from_secs(5), 
        "Consensus under adverse conditions took too long: {:?}", total_time);
    
    // Verify final state
    if let Some(confidence) = consensus.get_confidence(&vertex_id) {
        assert!(confidence.value > 0.5, 
            "Confidence too low despite consensus: {}", confidence.value);
    }
    
    // Check metrics for Byzantine detection
    let metrics = consensus.get_metrics();
    assert!(metrics.byzantine_behaviors_detected > 0, 
...
async fn test_complete_system_demonstration() {
    ...
    println!("✓ Network initialized with {} participants", consensus.participants.len());
    ...
    println!("✓ Added {} Byzantine participants (system remains fault tolerant)", 
    ...
    for i in 0..test_vertices {
        ...
        consensus.process_vertex(vertex_id.clone()).unwrap();
        let status = consensus.run_fast_consensus_round(&vertex_id).await.unwrap();
    ...
    // Test fork resolution
    let resolved_forks = consensus.detect_and_resolve_forks().unwrap();
    println!("✓ Fork resolution: {} conflicts resolved", resolved_forks.len());
    
    // Test Byzantine detection
    let detected_byzantine = consensus.detect_byzantine_patterns();
    println!("✓ Byzantine detection: {} malicious nodes identified", detected_byzantine.len());
    
    // Final metrics
    let metrics = consensus.get_metrics();
    println!("=== Final System Metrics ===");
    println!("Total vertices processed: {}", metrics.total_vertices_processed);
    println!("Current throughput: {:.2} vertices/sec", metrics.current_throughput);
    println!("Average finality time: {:?}", metrics.average_finality_time);
    println!("Byzantine behaviors detected: {}", metrics.byzantine_behaviors_detected);
    println!("Forks resolved: {}", metrics.forks_resolved);
    
    // Verify performance targets
    assert!(avg_finality < Duration::from_secs(1), 
        "Average finality time {:?} exceeds 1s target", avg_finality);
    assert!(metrics.current_throughput >= 10.0, 
        "Throughput {:.2} below 10 vertices/sec target", metrics.current_throughput);
