...
use crate::{Graph, Node, NodeState, QrAvalanche, ConsensusEvent, DagError};
...
async fn simulate_concurrent_voting(
    ...
    for i in 0..vote_count {
        ...
        let handle = tokio::spawn(async move {
            consensus.record_vote(node_hash, voter_hash, true).await
        });
...
async fn test_finality_latency() {
    ...
    for &node_hash in &node_hashes {
        ...
        simulate_concurrent_voting(&consensus, node_hash, CONCURRENT_VOTES).await.unwrap();
