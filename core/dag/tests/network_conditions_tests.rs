...
use qudag_dag::{QRAvalanche, QRAvalancheConfig, VertexId, ConsensusStatus, ConsensusError};
...
async fn test_network_partition_tolerance() {
    ...
    assert!(matches!(status_a, ConsensusStatus::Final | ConsensusStatus::Accepted));
    assert!(matches!(status_b, ConsensusStatus::Final | ConsensusStatus::Accepted));
...
async fn test_message_delay_tolerance() {
    ...
    // Add participants
    for i in 0..50 {
        let participant_id = VertexId::from_bytes(format!("delayed_participant_{}", i).into_bytes());
    ...
    assert!(achieved_consensus, "Failed to achieve consensus with message delays");
    
    // Should still be reasonably fast (under 3 seconds even with delays)
    assert!(total_time < Duration::from_secs(3), 
        "Consensus took too long with delays: {:?}", total_time);
}
...
async fn test_packet_loss_tolerance() {
        ...
        if effective_positive + effective_negative > 0 {
            ...
            if let Some(confidence) = consensus.confidence.get_mut(&vertex_id) {
                ...
                if confidence.value >= consensus.config.beta {
                    ...
                    assert!(successful_rounds as f64 / total_rounds as f64 >= 0.7, 
    ...
    assert!(successful_rounds > 0, "No successful rounds with packet loss simulation");
}
...
async fn test_jitter_tolerance() {
            ...
            if let Some(confidence) = consensus.confidence.get(&vertex_id) {
                if confidence.value >= consensus.config.beta {
                    ...
                    assert!(total_time < Duration::from_secs(2), 
                        "Consensus took too long with jitter: {:?}", total_time);
                    return;
                }
            }
        }
    ...
    if let Some(confidence) = consensus.confidence.get(&vertex_id) {
        assert!(confidence.value > 0.0, "No progress made with network jitter");
    }
}
...
async fn test_byzantine_under_network_stress() {
    ...
    assert!(metrics.total_vertices_processed > 0, "No vertices processed under stress");
}
...
async fn test_dynamic_network_changes() {
    ...
    for round in 0..10 {
        ...
        for i in 0..5 {
            let new_participant_id = VertexId::from_bytes(
                format!("dynamic_{}_round_{}", i, round).into_bytes()
            );
        ...
        if positive + negative > 0 {
            ...
            assert!(consensus.participants.len() >= initial_participants, 
...
async fn test_network_healing_recovery() {
    ...
    let main_status = consensus_main.run_consensus_round(&vertex_id).await.unwrap();
    ...
    assert!(matches!(main_status, ConsensusStatus::Final | ConsensusStatus::Accepted));
    ...
    consensus_main.process_vertex(new_vertex_id.clone()).unwrap();
    
    let post_healing_status = consensus_main.run_consensus_round(&new_vertex_id).await.unwrap();
    assert!(matches!(post_healing_status, ConsensusStatus::Final | ConsensusStatus::Accepted));
}