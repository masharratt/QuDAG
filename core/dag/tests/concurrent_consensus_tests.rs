...
use qudag_dag::{
    Graph, Node, QrDag, 
    consensus::{QrAvalanche, ConsensusEngine, Vote, VoteResult},
...
use tokio::sync::{Barrier, RwLock, Mutex};
...
async fn test_concurrent_dag_node_addition() {
    ...
    for thread_id in 0..NUM_THREADS {
        ...
        let handle = tokio::spawn(async move {
            ...
            (thread_id, successful_additions, failed_additions, node_hashes)
        });
    ...
    for handle in handles {
        ...
        println!("Thread {}: {} successful, {} failed node additions", 
    ...
    assert_eq!(final_dag_size, expected_size,
               "DAG size should match successful additions plus genesis");
    assert!(total_successful > 0, "Should have some successful additions");
    ...
    assert_eq!(accessible_count, all_node_hashes.len(),
...
async fn test_concurrent_consensus_voting() {
    ...
    for handle in handles {
        ...
        println!("Voter {}: {} votes cast, {} conflicts", voter_id, votes, conflicts);
        total_votes += votes;
    ...
    println!("  Conflict rate: {:.2}%", (total_conflicts as f64 / total_votes as f64) * 100.0);
    ...
    assert_eq!(total_votes, NUM_VOTERS * VOTES_PER_VOTER,
...
async fn test_node_state_transition_races() {
        ...
        let handle = tokio::spawn(async move {
            ...
            for i in 0..STATE_TRANSITIONS_PER_THREAD {
                ...
                let next_state = match current_state {
                    NodeState::Pending => {
                        if i % 3 == 0 { NodeState::Verified } else { NodeState::Processing }
                    }
                    NodeState::Processing => {
                        if i % 2 == 0 { NodeState::Verified } else { NodeState::Rejected }
                    }
                    ...
                    Ok(()) => {
                        ...
                        if let Some(updated_node) = dag_clone.get_node(&node_hash).await {
                            if updated_node.state() != next_state {
                                ...
                                eprintln!("Thread {}: State transition inconsistency for node {:?}", 
                                         thread_id, node_hash);
                            }
                        } else {
                            consistency_violations += 1;
                            consistency_clone.store(false, std::sync::atomic::Ordering::SeqCst);
                            eprintln!("Thread {}: Node disappeared after state transition", thread_id);
                        }
                    }
                    Err(e) => {
            ...
            (thread_id, successful_transitions, failed_transitions, consistency_violations)
        });
    ...
    for handle in handles {
        ...
        println!("Thread {}: {} successful, {} failed, {} violations", 
    ...
    assert!(total_successful > 0, "Should have some successful transitions");
...
async fn test_concurrent_tip_selection() {
    ...
    for selector_id in 0..NUM_SELECTORS {
        ...
        let handle = tokio::spawn(async move {
                    ...
                    Ok(tips) => {
                        ...
                        for tip in &tips {
                            if dag_clone.get_node(tip).await.is_none() {
                                eprintln!("Selector {}: Selected non-existent tip {:?}", selector_id, tip);
                                failed_selections += 1;
                            }
                        }
                    }
            ...
            (selector_id, successful_selections, failed_selections, unique_tips, tip_counts)
        });
    ...
    for handle in handles {
        ...
        println!("Selector {}: {} successful, {} failed, {} unique tips", 
                selector_id, successful, failed, unique_tips.len());
    ...
    assert!(total_successful > 0, "Should have successful tip selections");
    ...
    for tip in &all_unique_tips {
        assert!(dag.get_node(tip).await.is_some(), 
...
async fn test_dag_high_contention_stress() {
            ...
            while Instant::now() < end_time {
                ...
                let parents = {
                    ...
                    if nodes.is_empty() {
                        ...
                        nodes.choose_multiple(&mut thread_rng(), count).cloned().collect()
                    }
                };
    ...
    for updater_id in 0..NUM_STATE_UPDATERS {
        ...
        let handle = tokio::spawn(async move {
            ...
            while Instant::now() < end_time {
                ...
                if let Some(node) = dag_clone.get_node(&target_hash).await {
                    ...
                    let next_state = match current_state {
                        ...
                        NodeState::Processing => {
                            if operations % 2 == 0 { NodeState::Verified } else { NodeState::Rejected }
                        }
                        NodeState::Verified => NodeState::Final,
                        _ => continue, // Can't transition further
                    };
                    
                    if dag_clone.update_node_state(&target_hash, next_state).await.is_ok() {
            ...
            (format!("Updater_{}", updater_id), operations, successful_updates)
        });
    ...
    for reader_id in 0..NUM_READERS {
        ...
        let handle = tokio::spawn(async move {
            ...
            (format!("Reader_{}", reader_id), operations, successful_reads)
        });
    ...
    for handle in handles {
        ...
        println!("{}: {} operations, {} specific metric", task_name, ops, specific_metric);
    ...
    assert_eq!(final_counter, total_operations, "Counter should match operations");
    assert!(final_consistency, "Should maintain consistency");
    assert_eq!(final_dag_size, final_node_list_size, "DAG size should match node list");
    assert!(ops_per_second > 50.0, "Should achieve reasonable throughput");
}
...
fn test_dag_parallel_operations() {
    ...
    rt.block_on(async {
        ...
        println!("  Successful additions: {}/{}", successful_additions, NUM_OPERATIONS);
        println!("  Final DAG size: {}", dag.node_count().await);
        
        // Verify all nodes are accessible
        let mut accessible_count = 0;
        let verification_handles: Vec<_> = added_hashes.into_iter().map(|hash| {
            let dag_clone = dag.clone();
            tokio::spawn(async move {
                dag_clone.get_node(&hash).await.is_some()
            })
        }).collect();
        ...
        println!("  Accessible nodes: {}/{}", accessible_count, successful_additions);
        
        assert_eq!(successful_additions, NUM_OPERATIONS, "All operations should succeed");
        assert_eq!(accessible_count, successful_additions, "All nodes should be accessible");
        assert_eq!(dag.node_count().await, successful_additions + 1, "DAG size should be correct");
    });
}