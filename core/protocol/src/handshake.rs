...
use qudag_crypto::{
    ...
    MlKem768, KeyPair as KemKeyPair, PublicKey as KemPublicKey, SecretKey,
    SharedSecret, Ciphertext as KemCiphertext
};
use rand;

use crate::message::{
    Message, MessageType, HandshakeType, ProtocolVersion, 
    MessageError
};
...
impl HandshakeCoordinator {
    ...
    pub fn generate_keys() -> Result<HandshakeKeys, HandshakeError> {
        ...
        let signature_keypair = MlDsaKeyPair::generate(&mut rand::thread_rng())
            .map_err(|e| HandshakeError::CryptoError { 
                reason: format!("Failed to generate ML-DSA keypair: {:?}", e) 
            })?;
        
        // Generate ML-KEM keypair
        let (kem_public_key, kem_secret_key) = MlKem768::keygen()
            .map_err(|e| HandshakeError::CryptoError {
    ...
    pub fn initiate_handshake(
        ...
        let payload_bytes = bincode::serialize(&payload)
            .map_err(|e| HandshakeError::InvalidMessage { 
    ...
    pub fn process_handshake_message(
        ...
        match &message.msg_type {
            MessageType::Handshake(HandshakeType::Init) => {
                self.process_handshake_init(message)
            }
    ...
    fn process_handshake_init(&mut self, message: &Message) -> Result<Option<Message>, HandshakeError> {
        let payload: HandshakeMessagePayload = bincode::deserialize(&message.payload)
            .map_err(|e| HandshakeError::InvalidMessage { 
        ...
        if let HandshakeMessagePayload::Init { 
            ...
            let negotiated_version = self.negotiate_version(&supported_versions, &protocol_version)?;
            
            // Verify required capabilities
            self.verify_capabilities(&capabilities)?;
            
            // Parse peer keys
            let peer_signature_key = MlDsaPublicKey::from_bytes(&signature_public_key)
                .map_err(|e| HandshakeError::CryptoError { 
                    reason: format!("Invalid peer signature key: {:?}", e) 
                })?;
            
            let peer_kem_key = KemPublicKey::from_bytes(&kem_public_key)
                .map_err(|e| HandshakeError::CryptoError { 
            ...
            let (kem_ciphertext, shared_secret) = MlKem768::encapsulate(&peer_kem_key)
                .map_err(|e| HandshakeError::CryptoError {
            ...
            let response_bytes = bincode::serialize(&response_payload)
                .map_err(|e| HandshakeError::InvalidMessage { 
            ...
            self.state_machine.process_message(message, Some(session_id))?;
            
            info!("Processed handshake init, sending response for session: {}", session_id);
    ...
    fn process_handshake_response(
        ...
        if let HandshakeMessagePayload::Response { 
            ...
            let kem_ciphertext_bytes = KemCiphertext::from_bytes(&kem_ciphertext)
                .map_err(|e| HandshakeError::CryptoError {
            ...
            let shared_secret = MlKem768::decapsulate(&secret_key, &kem_ciphertext_bytes)
                .map_err(|e| HandshakeError::CryptoError {
            ...
            let shared_secrets = {
                ...
                let mac_key = blake3::keyed_hash(
                    &blake3::hash(b"QuDAG-MAC-Key").as_bytes(),
                    &key_material,
                ).as_bytes().to_vec();
            ...
            info!("Processed handshake response, sending completion for session: {}", session_id);
    ...
    fn process_handshake_complete(
        ...
        info!("Handshake completed successfully for session: {}", session_id);
    ...
    fn process_version_negotiation(
        ...
        if let HandshakeMessagePayload::VersionNegotiation { 
            ...
            let compatible_version = self.negotiate_version(&supported_versions, &preferred_version)?;
    ...
    pub fn cleanup_sessions(&mut self) {
        ...
        self.sessions.retain(|_, session| {
            let elapsed = now.duration_since(session.started_at).unwrap_or(Duration::ZERO);
    ...
    pub fn get_active_sessions(&self) -> Vec<&HandshakeSession> {
        self.sessions.values()
            .filter(|s| !matches!(s.state, HandshakeSessionState::Completed | HandshakeSessionState::Failed))
            .collect()
    }
