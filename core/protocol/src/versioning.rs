...
use crate::message::{ProtocolVersion, Message, MessageType};
...
pub enum VersionError {
    ...
    IncompatibleVersions { local: ProtocolVersion, remote: ProtocolVersion },
    ...
    MigrationFailed { from: ProtocolVersion, to: ProtocolVersion, reason: String },
...
pub enum MigrationStrategy {
    ...
    Custom(Box<dyn Fn(&Message) -> Result<Message, VersionError> + Send + Sync>),
    /// Not supported
    NotSupported,
}
...
impl VersionRegistry {
    ...
    fn register_default_versions(&mut self) {
        ...
        let v1_0_0 = VersionInfo {
            version: ProtocolVersion { major: 1, minor: 0, patch: 0, features: vec![] },
            ...
            security_requirements: SecurityRequirements {
                min_key_sizes: [
                    ("ml-dsa".to_string(), 2048),
                    ("ml-kem".to_string(), 768),
                ].into_iter().collect(),
            ...
            compatibility: CompatibilityInfo {
                compatible_with: vec![],
                min_supported_version: ProtocolVersion { major: 1, minor: 0, patch: 0, features: vec![] },
        ...
        let v1_1_0 = VersionInfo {
            version: ProtocolVersion { major: 1, minor: 1, patch: 0, features: vec![] },
    ...
    pub fn is_supported(&self, version: &ProtocolVersion) -> bool {
        self.supported_versions.iter().any(|v| &v.version == version)
    }
    
    /// Get version information
    pub fn get_version_info(&self, version: &ProtocolVersion) -> Option<&VersionInfo> {
        self.supported_versions.iter().find(|v| &v.version == version)
    }
...
impl VersionManager {
    ...
    pub fn new(current_version: ProtocolVersion) -> Self {
        let preferences = VersionPreferences {
            ...
            min_version: ProtocolVersion { major: 1, minor: 0, patch: 0, features: vec![] },
            max_version: ProtocolVersion { major: 1, minor: 1, patch: 0, features: vec![] },
    ...
    pub fn negotiate_version(
        ...
        // Find best compatible version
        if let Some(best_version) = self.registry.find_best_compatible_version(
            &available_versions,
            &self.preferences,
        ) {
            Ok(best_version)
    ...
    pub fn migrate_message(
        ...
        if let Some(strategy) = self.registry.get_migration_strategy(from_version, to_version) {
            match strategy {
    ...
    pub fn is_feature_available(&self, feature: &str) -> bool {
        self.registry.is_feature_supported(&self.current_version, feature)
    }
...
fn downgrade_1_1_to_1_0(message: &Message) -> Result<Message, VersionError> {
    ...
    match &message.msg_type {
        MessageType::Anonymous(_) => {
            ...
            migrated_message.msg_type = MessageType::Routing(
...
mod tests {
    ...
    fn test_version_negotiation() {
        ...
        let peer_preferred = ProtocolVersion { major: 1, minor: 1, patch: 0, features: vec![] };
        
        let negotiated = manager.negotiate_version(&peer_versions, &peer_preferred).unwrap();
