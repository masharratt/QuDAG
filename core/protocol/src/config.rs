...
impl Config {
    ...
    pub fn apply_env_overrides(&mut self) -> Result<(), ConfigError> {
        ...
        if let Ok(max_peers) = env::var("QUDAG_MAX_PEERS") {
            self.network.max_peers = max_peers.parse()
        ...
        if let Ok(threshold) = env::var("QUDAG_FINALITY_THRESHOLD") {
            self.consensus.finality_threshold = threshold.parse()
        ...
        if let Ok(max_rounds) = env::var("QUDAG_MAX_ROUNDS") {
            self.consensus.max_rounds = max_rounds.parse()
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.node.node_id.is_empty() {
            return Err(ConfigError::InvalidValue("node_id cannot be empty".to_string()));
        }
        
        if self.node.log_level.is_empty() {
            return Err(ConfigError::InvalidValue("log_level cannot be empty".to_string()));
        }
        ...
        match self.node.log_level.as_str() {
            ...
            _ => return Err(ConfigError::InvalidValue(
                format!("Invalid log_level: {}", self.node.log_level)
            )),
        }
        ...
        if self.network.port > 65535 {
            return Err(ConfigError::InvalidValue("port must be <= 65535".to_string()));
        }
        
        if self.network.max_peers == 0 {
            return Err(ConfigError::InvalidValue("max_peers must be > 0".to_string()));
        }
        
        if self.network.max_peers > 10000 {
            return Err(ConfigError::InvalidValue("max_peers must be <= 10000".to_string()));
        }
        ...
        if self.network.connect_timeout > Duration::from_secs(300) {
            return Err(ConfigError::InvalidValue("connect_timeout must be <= 300s".to_string()));
        }
        ...
        if self.consensus.round_timeout.is_zero() {
            return Err(ConfigError::InvalidValue("round_timeout must be > 0".to_string()));
        }
        
        if self.consensus.max_rounds == 0 {
            return Err(ConfigError::InvalidValue("max_rounds must be > 0".to_string()));
        }
        
        if self.consensus.max_rounds > 1000 {
            return Err(ConfigError::InvalidValue("max_rounds must be <= 1000".to_string()));
        }
    ...
    pub fn save_to_toml<P: AsRef<std::path::Path>>(&self, path: P) -> Result<(), ConfigError> {
        let content = toml::to_string_pretty(self)
            .map_err(|e| ConfigError::SerializationError(serde_json::Error::io(std::io::Error::new(std::io::ErrorKind::InvalidData, e))))?;
        fs::write(path, content)?;
        Ok(())
    }
}