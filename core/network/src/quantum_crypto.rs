...
impl MlKem {
    ...
    pub fn generate_keypair(&mut self) -> Result<(MlKemPublicKey, MlKemSecretKey), NetworkError> {
        info!("Generating ML-KEM keypair with security level: {:?}", self.security_level);
    ...
    pub fn encapsulate(&mut self, public_key: &MlKemPublicKey) -> Result<(MlKemCiphertext, SharedSecret), NetworkError> {
    ...
    pub fn decapsulate(&self, secret_key: &MlKemSecretKey, ciphertext: &MlKemCiphertext) -> Result<SharedSecret, NetworkError> {
...
impl QuantumKeyExchange {
    ...
    pub fn complete_exchange(&self, ciphertext: &MlKemCiphertext) -> Result<SharedSecret, NetworkError> {
        ...
        let (_, secret_key) = self.our_keypair.as_ref()
...
pub mod utils {
    ...
    pub fn derive_keys(shared_secret: &SharedSecret, info: &[u8], key_count: usize) -> Vec<[u8; 32]> {
    ...
    pub fn combine_secrets(secrets: &[&SharedSecret]) -> SharedSecret {
        if secrets.is_empty() {
            return SharedSecret { secret: vec![0u8; 32] };
        }
