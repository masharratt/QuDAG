...
impl DarkDomainRecord {
    ...
    fn sign(&mut self, keypair: &MlDsaKeyPair) -> Result<(), DarkResolverError> {
        ...
        self.signature = keypair.sign(&message, &mut rng)
    ...
    fn to_signable_bytes(&self) -> Result<Vec<u8>, DarkResolverError> {
        ...
        for addr in &self.addresses {
            hasher.update(&bincode::serialize(addr).map_err(|e| 
                DarkResolverError::CryptoError(e.to_string()))?); 
        }
        ...
        hasher.update(&bincode::serialize(&self.owner_id).map_err(|e| 
            DarkResolverError::CryptoError(e.to_string()))?);
...
impl DarkResolver {
    ...
    pub fn register_domain<R: CryptoRng + RngCore>(
        ...
        {
            let mut reverse = self.reverse_lookup.write()
                .map_err(|_| DarkResolverError::StorageError)?;
        ...
        // Remove expired domains
        {
            ...
            for domain in to_remove {
                if let Some(record) = domains.remove(&domain) {
                    ...
                    let addr = Self::generate_dark_address(
...
mod tests {
    ...
    fn test_valid_dark_domains() {
        ...
        assert!(!DarkResolver::is_valid_dark_domain(&format!("{}.dark", "a".repeat(64)))); // too long
    }
