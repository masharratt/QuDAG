                            ...
                            if j % BATCH_SIZE == 0 {
                                ...
                                for k in 0..BATCH_SIZE.min(messages_per_sender - j) {
                                    let batch_peer = PeerId::from_bytes({
                                        let mut id = [0u8; 32];
                                        id[0..4].copy_from_slice(&(peer_base + j + k).to_le_bytes());
                                        id
                                    });
                                ...
                                for batch_peer in batch_peers {
                                    ...
                                    black_box(manager.update_status(batch_peer, ConnectionStatus::Connected));
                                }
                            }
                        }
                    });
    ...
    c.bench_function("optimized_connection_pooling", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let setup_handles: Vec<_> = test_peers.chunks(100).map(|chunk| {
                    let manager = manager.clone();
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                for i in 0..50 {
                    ...
                    manager.update_status(peer, ConnectionStatus::Failed("Simulated failure".into()));
                }
