...
use qudag_network::{Router, PeerId, NetworkMessage, MessagePriority, RoutingStrategy};
...
fn bench_route_computation(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("anonymous_route", size), size, |b, &size| {
...
fn bench_circuit_setup(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("setup", size), size, |b, &size| {
            ...
            b.to_async(&rt).iter(|| async {
                black_box(router.setup_circuit(
                    &source,
                    &dest,
                    3 // hops
                ).await.unwrap())
...
fn bench_message_routing(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("route_message", size), size, |b, &size| {
            ...
            b.to_async(&rt).iter(|| async {
                black_box(router.send_message(msg.clone()).await.unwrap())
            });
        });
    }
