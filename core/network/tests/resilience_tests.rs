use qudag_network::{
    ...
    RoutingStrategy, Router, ConnectionStatus, QueueMetrics, LatencyMetrics, ThroughputMetrics
};
...
async fn test_high_load_resilience() {
        ...
        let handle = tokio::spawn(async move {
            ...
            for j in 0..messages_per_peer {
                let result = timeout(Duration::from_millis(100), async {
                    manager_clone.update_metrics(1000.0 + j as f64, 10 + (j % 100) as u64).await;
                }).await;
    ...
    println!("High load test: {} peers, {} completed in {:?}", 
             num_peers, completed, elapsed);
    
    // Verify system remained stable
    assert!(completed >= num_peers / 2, "Too many tasks failed: {}/{}", completed, num_peers);
...
async fn test_connection_failure_recovery() {
    ...
    for (i, peer_id) in peers.iter().enumerate() {
        if i % 3 == 0 { // Fail every third connection
            manager.update_status(*peer_id, ConnectionStatus::Failed("Simulated failure".into())).await;
...
async fn test_routing_peer_churn() {
    ...
    println!("Routing success rate with peer churn: {:.2}%", success_rate * 100.0);
...
async fn test_backpressure_handling() {
    ...
    for i in 0..1000 {
        ...
        if i > 100 && i % 100 == 0 {
            ...
            if recent_avg > early_avg * 2 {
                println!("Backpressure detected at iteration {}: {:?} vs {:?}", 
    ...
    println!("Backpressure test: {} updates in {:?}, avg: {:?}", 
             update_times.len(), total_time, avg_update_time);
...
async fn test_graceful_degradation() {
    ...
    assert!(successful_connections >= 3, "Too few successful connections: {}", successful_connections);
...
async fn test_split_brain_handling() {
    ...
    for peer in &shared_peers {
        ...
        manager1.update_status(*peer, ConnectionStatus::Connected).await;
        manager2.update_status(*peer, ConnectionStatus::Connected).await;
    }
...
async fn test_metrics_consistency() {
    ...
    for i in 0..num_updaters {
        ...
        let handle = tokio::spawn(async move {
            ...
            for j in 0..updates_per_task {
                ...
                manager_clone.update_metrics(messages_per_sec, latency_ms).await;
    ...
    println!("Final metrics: {:.2} msg/s, {} connections, {:?} latency", 
             final_metrics.messages_per_second, 
             final_metrics.connections, 
...
async fn test_router_concurrent_consistency() {
                ...
                match j % 4 {
                    ...
                    1 => {
                        ...
                        if !local_peers.is_empty() {
                            ...
                            let _ = router_clone.route(&msg, RoutingStrategy::Anonymous { hops: 2 }).await;
                        }
                    }
                    2 => {
                        // Get hop info
    ...
    println!("Concurrent router test: {} peers added across {} tasks", 
