use qudag_network::{
    NetworkMessage, PeerId, MessagePriority, RoutingStrategy, Router, HopInfo,
    ConnectionManager, ConnectionStatus, MessageEnvelope, MessageQueue, NetworkError
};
...
async fn test_anonymous_routing_properties() {
    ...
    for _ in 0..10 {
        let route = router.route(&msg, RoutingStrategy::Anonymous { hops: 5 }).await.unwrap();
        routes.push(route);
    }
    ...
    for route in &routes {
        ...
        assert!(!route.contains(&dest_peer), "Route contains destination peer");
        ...
        assert_eq!(unique_peers.len(), route.len(), "Route contains duplicate peers");
    }
    
    // Routes should be different (probabilistically)
    let unique_routes: HashSet<_> = routes.iter().collect();
    assert!(unique_routes.len() > 1, "All routes are identical - not sufficiently random");
}
...
async fn test_hop_isolation() {
    ...
    // Create route
    let route = router.route(&msg, RoutingStrategy::Anonymous { hops: 4 }).await.unwrap();
    
    // Verify hop isolation properties
    for (i, peer_id) in route.iter().enumerate() {
        let hop_info = router.get_hop_info(peer_id).await;
        assert!(hop_info.is_ok(), "Missing hop info for peer at position {}", i);
        
        let info = hop_info.unwrap();
        
        // Each hop should only be able to decrypt its own layer
        assert!(info.can_decrypt_layer(i), "Hop {} cannot decrypt its own layer", i);
        
        // Hop should not be able to decrypt other layers
        for j in 0..route.len() {
            if j != i {
                assert!(!info.can_decrypt_layer(j), 
                    "Hop {} can decrypt layer {} (should only decrypt layer {})", i, j, i);
            }
        }
        ...
        for peer in &route {
            if expected_known_peers.contains(peer) {
                assert!(info.knows_peer(peer), 
                    "Hop {} should know about adjacent peer", i);
            } else if *peer != *peer_id {
                assert!(!info.knows_peer(peer), 
                    "Hop {} should not know about non-adjacent peer", i);
            }
        }
    }
}
...
async fn test_traffic_analysis_resistance() {
    ...
    for i in 0..50 {
        let msg = NetworkMessage {
            ...
            priority: if i % 3 == 0 { MessagePriority::High } else { MessagePriority::Normal },
    ...
    assert!(*max_frequency <= max_allowed_frequency, 
        "Peer appears too frequently in routes: {} out of {} total hops", 
        max_frequency, total_hops);
    ...
    assert!(diversity_ratio >= 0.7, 
        "Route diversity too low: {:.2}% unique routes", diversity_ratio * 100.0);
}
...
async fn test_timing_attack_resistance() {
    ...
    for _ in 0..100 {
        ...
        let _route = router.route(&msg, RoutingStrategy::Anonymous { hops: 3 }).await.unwrap();
    ...
    let variance = routing_times.iter()
        .map(|t| {
    ...
    println!("Timing analysis - Avg: {:?}, Min: {:?}, Max: {:?}, CV: {:.3}", 
    ...
    assert!(max_ratio < 10.0, "Extreme timing difference detected: {:.2}x", max_ratio);
}
...
async fn test_connection_metadata_protection() {
    ...
    for i in 0..10 {
        ...
        manager.update_status(peer_id, ConnectionStatus::Connected).await;
...
async fn test_statistical_analysis_resistance() {
    ...
    for (source_idx, dest_idx, route) in &all_routes {
        ...
        source_route_correlation.entry(*source_idx).or_insert_with(Vec::new).push(route_pattern.clone());
        
        // Track correlation between destination and route
        dest_route_correlation.entry(*dest_idx).or_insert_with(Vec::new).push(route_pattern);
    }
...
async fn test_forward_secrecy() {
    ...
    // Route first message
    let route1 = router.route(&msg1, RoutingStrategy::Anonymous { hops: 3 }).await.unwrap();
    ...
    let route2 = router.route(&msg2, RoutingStrategy::Anonymous { hops: 3 }).await.unwrap();
...
async fn test_message_unlinkability() {
    ...
    for (dest1, routes1) in &routes_by_dest {
        for (dest2, routes2) in &routes_by_dest {
            if dest1 >= dest2 { continue; } // Avoid duplicate comparisons
    ...
    let avg_common_peers = common_peers_counts.iter().sum::<usize>() as f64 / common_peers_counts.len() as f64;
    ...
    println!("Average common peers between destination routes: {:.2}", avg_common_peers);
    println!("Overlap ratio: {:.2}%", overlap_ratio * 100.0);
    
    // Overlap should be reasonable (not too high to maintain unlinkability)
    assert!(overlap_ratio < 0.7, "Too much overlap between routes to different destinations: {:.2}%", overlap_ratio * 100.0);
}
...
async fn test_nonce_reuse_vulnerability() {
    ...
    for i in 0..1000 {
        ...
        if message_hashes.contains(&envelope.hash) {
            panic!("CRITICAL: Duplicate message hash detected - nonce reuse vulnerability confirmed");
        }
        
        message_hashes.insert(envelope.hash);
    }
    
    println!("Nonce uniqueness test passed - {} unique hashes generated", message_hashes.len());
}
...
async fn test_message_authentication_bypass() {
    ...
    assert!(envelope.verify(), "Original message should verify correctly");
    ...
    match result {
        Ok(true) => panic!("CRITICAL: Unsigned message accepted as valid - signature bypass vulnerability"),
...
async fn test_queue_dos_vulnerability() {
            ...
            Ok(Ok(_)) => {
                ...
                if i % 10000 == 0 {
                    ...
                    if queue_size > 150_000 {
                        panic!("CRITICAL: Queue grew unboundedly to {} - DoS vulnerability confirmed", queue_size);
                    }
                }
            }
            Ok(Err(_)) => {
    ...
    println!("DoS test completed - {} messages accepted before protection kicked in", messages_accepted);
...
async fn test_connection_pool_dos() {
    ...
    for i in 0..100 {
        ...
        if successful_connections > 15 { // Allow some margin for async behavior
            panic!("CRITICAL: Connection limit not enforced - {} connections accepted", successful_connections);
        }
    }
    
    println!("Connection limit test passed - {} connections accepted", successful_connections);
}
...
async fn test_replay_attack_vulnerability() {
    ...
    // Send the message
    assert!(queue.enqueue(msg.clone()).await.is_ok(), "First message should be accepted");
...
async fn test_peer_identity_spoofing() {
    ...
    if weak_ids > 0 {
        panic!("CRITICAL: {} weak peer IDs generated out of 1000 - randomness vulnerability", weak_ids);
    }
...
async fn test_weak_encryption_implementation() {
    ...
    if duplicates > 0 {
        panic!("CRITICAL: {} duplicate keys generated - weak randomness in key generation", duplicates);
    }
    ...
    for (byte_val, &freq) in byte_frequencies.iter().enumerate() {
        if freq < threshold || freq > expected_freq + threshold {
            ...
            if freq == 0 || freq > expected_freq * 2 {
                println!("WARNING: Byte value {} has suspicious frequency: {}", byte_val, freq);
            }
        }
    }
    
    if biased_bytes > 50 { // More than ~20% of bytes showing bias
        println!("WARNING: {} byte values show significant bias - potential weak randomness", biased_bytes);
    }
    
    println!("Encryption entropy test completed - {} keys tested", keys.len());
}
...
async fn test_large_message_dos() {
    ...
    for size_mb in 1..=20 {
        ...
        match timeout(Duration::from_millis(100), queue.enqueue(msg)).await {
            ...
            Err(_) => {
                println!("Timeout on {}MB message - system may be overwhelmed", size_mb);
                break;
            }
        }
    }
...
async fn test_concurrent_attack_scenarios() {
    ...
    println!("Concurrent attack test: {} messages, {} connections accepted", 
    ...
    if total_connections > 100 {
        panic!("CRITICAL: Too many connections accepted under concurrent attack - DoS vulnerability");
    }
    ...
    println!("Final state: {} queued messages, {} active connections", 
