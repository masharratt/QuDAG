...
use qudag_network::{
    shadow_address::*,
    dark_resolver::{DarkResolver, DarkDomainRecord},
...
async fn test_shadow_address_with_dark_resolver() {
    ...
    let stealth_addr = handler.generate_stealth_address(
        network,
        &recipient_view,
        &recipient_spend,
    ).unwrap();
    ...
    resolver.register_domain("shadow-test.dark", network_addr.clone()).unwrap();
...
async fn test_address_pool_rotation_with_network() {
    ...
    manager.create_address_pool("network_pool".to_string(), 10, Some(pool_ttl)).await.unwrap();
...
async fn test_hierarchical_address_derivation_for_channels() {
    ...
    for i in 0..channel_addresses.len() {
        for j in (i + 1)..channel_addresses.len() {
            ...
            assert_ne!(channel_addresses[i].spend_key, channel_addresses[j].spend_key);
        }
    }
...
async fn test_usage_based_rotation() {
    ...
    *manager.rotation_policies.write().await = policies;
    ...
    // Create pool
    manager.create_address_pool("usage_pool".to_string(), 5, None).await.unwrap();
...
async fn test_stealth_address_with_network_messages() {
    ...
    let stealth = handler.generate_stealth_address(
        NetworkType::Testnet,
        &recipient_view,
        &recipient_spend,
    ).unwrap();
