...
use qudag_network::{
    connection::{ConnectionInfo, ConnectionManager, HealthStatistics, SecureConfig, TransportKeys},
    ...
    onion::{MLKEMOnionRouter, MetadataConfig, MetadataProtector, MixConfig, MixNode, OnionError, OnionLayer, TrafficAnalysisConfig},
    peer::{PeerInfo, PeerManager, PeerStatus},
    quantum_crypto::{MlKemSecurityLevel, QuantumKeyExchange},
    router::{HopInfo, Router},
    shadow_address::{DefaultShadowAddressHandler, NetworkType, ShadowAddress, ShadowAddressGenerator},
    transport::{AsyncTransport, Transport, TransportConfig},
    types::{ConnectionStatus, MessagePriority, NetworkError, NetworkMessage, PeerId, RoutingStrategy},
...
mod connection_tests {
    ...
    async fn test_connection_manager_lifecycle() {
        ...
        assert_eq!(manager.get_status(&peer_id), Some(ConnectionStatus::Connected));
    ...
    async fn test_secure_config_generation() {
        ...
        assert_ne!(config.transport_keys.public_key, config.transport_keys.private_key);
    }
    ...
    async fn test_connection_auto_recovery() {
        ...
        assert_eq!(manager.get_status(&peers[0]), Some(ConnectionStatus::Connected));
        assert_eq!(manager.get_status(&peers[1]), Some(ConnectionStatus::Connected));
    }
}
...
mod peer_tests {
    ...
    async fn test_peer_manager_operations() {
        ...
        assert!(manager.update_peer_status(peer_id, PeerStatus::Disconnected).await.is_ok());
...
mod router_tests {
    ...
    async fn test_routing_strategies() {
        ...
        let direct_route = router
            .route(&msg, RoutingStrategy::Direct)
            .await
            .unwrap();
...
mod onion_tests {
    ...
    async fn test_onion_layer_creation() {
        ...
        for i in 1..onion_layers.len() {
            assert!(onion_layers[i].encrypted_data.len() >= onion_layers[i-1].encrypted_data.len());
        }
    }
    ...
    async fn test_onion_layer_peeling() {
        ...
        // Create onion layers
        let layers = router.create_layers(&original_payload, &peer_keys).await.unwrap();
    ...
    async fn test_mix_node_operations() {
        ...
        let original_order: Vec<_> = (0..5).map(|i| format!("message_{}", i).into_bytes()).collect();
...
mod quantum_crypto_tests {
    ...
    async fn test_quantum_key_exchange() {
        ...
        for level in [MlKemSecurityLevel::Level1, MlKemSecurityLevel::Level3, MlKemSecurityLevel::Level5] {
    ...
    async fn test_quantum_performance() {
        ...
        assert!(ops_per_sec > 10.0, "Quantum crypto performance too slow: {} ops/sec", ops_per_sec);
    }
}
...
mod shadow_address_tests {
    ...
    async fn test_shadow_address_privacy() {
        ...
        for i in 0..addresses.len() - 1 {
            ...
            let common_prefix_len = addr1.chars()
                .zip(addr2.chars())
                .take_while(|(a, b)| a == b)
                .count();
...
mod network_manager_tests {
    ...
    async fn test_network_manager_initialization() {
        ...
        assert_eq!(configured_manager.config.connection_timeout, Duration::from_secs(15));
    }
...
async fn test_comprehensive_integration() {
    ...
    for peer in &route {
        assert_eq!(connection_manager.get_status(peer), Some(ConnectionStatus::Connecting));
    }
