...
use qudag_network::{
    connection::{ConnectionManager, SecureConnection, SecureConfig, TransportKeys},
    types::{PeerId, ConnectionStatus, NetworkError, NetworkMetrics},
    peer::{PeerManager, PeerInfo, PeerStatus},
    router::{Router, Route, RoutingTable},
    message::{Message, MessageType, MessageHeader},
    p2p::P2PManager,
};
use std::sync::Arc;
use std::time::{Duration, Instant};
use std::collections::HashMap;
use tokio::sync::{Barrier, Semaphore, RwLock, Mutex};
...
async fn test_concurrent_connection_management() {
    ...
    for thread_id in 0..NUM_THREADS {
        ...
        let handle = tokio::spawn(async move {
            ...
            (thread_id, successful_connections, successful_disconnections, status_updates)
        });
    ...
    for handle in handles {
        ...
        println!("Thread {}: {} connections, {} disconnections, {} status updates", 
    ...
    println!("Total: {} connections, {} disconnections, {} status updates", 
    ...
    println!("Final connection count: {}, expected: {}", final_connection_count, expected_connections);
    
    // The actual count may be limited by MAX_CONNECTIONS
    assert!(final_connection_count <= MAX_CONNECTIONS, 
           "Should not exceed max connections");
    assert!(total_connections > 0, "Should have some successful connections");
}
...
async fn test_concurrent_message_processing() {
    ...
    println!("Total produced: {}, consumed: {}, processed counter: {}, remaining: {}", 
    ...
    assert_eq!(total_consumed, final_processed,
               "Consumed count should match processed counter");
    assert_eq!(total_consumed + remaining_messages, total_produced,
...
async fn test_concurrent_peer_management() {
        ...
        let handle = tokio::spawn(async move {
            ...
            for i in 0..PEERS_PER_THREAD {
                ...
                if manager_clone.add_peer(peer_info).await.is_ok() {
                    ...
                    // Update status
                    if manager_clone.update_peer_status(peer_id, PeerStatus::Connected).await.is_ok() {
                        status_updates += 1;
                    }
                }
    ...
    for handle in handles {
        ...
        println!("Thread {}: {} added, {} status updates, {} removed", 
    ...
    println!("Total: {} added, {} removed, {} status updates", 
            total_added, total_removed, total_status_updates);
    println!("Final peer count: {}, expected: {}", final_peer_count, expected_peers);
    
    assert_eq!(final_peer_count, expected_peers,
...
async fn test_concurrent_routing_operations() {
    ...
    for handle in handles {
        ...
        println!("Thread {}: {} routes added, {} found, {} removed", 
    ...
    println!("Total: {} added, {} found, {} removed, final count: {}", 
            total_added, total_found, total_removed, final_route_count);
    
    assert_eq!(final_route_count, expected_routes,
               "Final route count should match expectations");
    assert!(total_added > 0, "Should add some routes");
    assert_eq!(total_found, total_added, "Should find all added routes initially");
}
...
async fn test_network_high_concurrency_stress() {
        ...
        let handle = tokio::spawn(async move {
            ...
            while Instant::now() < end_time {
                if operations % 3 == 0 {
                    ...
                    if operations % 2 == 0 {
                        let _ = manager_clone.update_peer_status(peer_id, PeerStatus::Disconnected).await;
                    } else {
                        if manager_clone.remove_peer(&peer_id).await.is_ok() {
                            peers.remove(index);
                        }
    ...
    println!("  Final connections: {}", connection_manager.connection_count());
    ...
    assert!(ops_per_second > 100.0, "Should achieve reasonable throughput");
...
async fn test_thread_safe_data_structures() {
                ...
                if manager_clone.connect(peer_id).await.is_ok() {
                    ...
                    if let Some(status) = manager_clone.get_status(&peer_id) {
                        if status != ConnectionStatus::Connected {
                            ...
                            eprintln!("Writer {}: Consistency check failed for status update", writer_id);
                        }
                    } else {
                        check_clone.store(false, std::sync::atomic::Ordering::SeqCst);
                        eprintln!("Writer {}: Consistency check failed - peer not found after add", writer_id);
                    }
                    ...
                    if i % 10 == 0 {
                        ...
                        if manager_clone.get_status(&peer_id).is_some() {
                            check_clone.store(false, std::sync::atomic::Ordering::SeqCst);
                            eprintln!("Writer {}: Consistency check failed - peer found after removal", writer_id);
                        }
                    }
                }
        ...
        let handle = tokio::spawn(async move {
            ...
            for i in 0..OPERATIONS_PER_TASK {
                ...
                if metrics_after.active_connections == count_after {
                    ...
                    check_clone.store(false, std::sync::atomic::Ordering::SeqCst);
                    eprintln!("Reader {}: Metrics inconsistency - active: {}, count: {}", 
            ...
            (format!("Reader_{}", reader_id), operations, consistency_checks)
        });
    ...
    for handle in handles {
        ...
        match result {
            (name, ops, checks) => {
                println!("{}: {} operations, {} consistency checks", name, ops, checks);
    ...
    println!("  Final connections: {}", final_connections);
    println!("  Metrics active connections: {}", final_metrics.active_connections);
    
    assert!(final_consistency, "Data structures should maintain consistency");
    assert_eq!(final_connections, final_metrics.active_connections,
