...
use qudag_crypto::{
    ...
    fingerprint::{PeerFingerprint, generate_peer_fingerprint},
...
use qudag_dag::{
    ...
    dag::{DAG, DagConfig, Transaction, Vertex},
    node::{Node, NodeConfig, NodeId},
    tip_selection::{TipSelector, TipSelectionStrategy},
};
use qudag_network::{
...
async fn test_network_mldsa_integration() {
    ...
    assert!(alice_keypair.public_key().verify(&received_hash.as_bytes(), &signature));
    
    // Test signature verification failure with wrong key
    assert!(!bob_keypair.public_key().verify(&received_hash.as_bytes(), &signature));
    ...
    assert!(!alice_keypair.public_key().verify(&tampered_hash.as_bytes(), &signature));
}
...
async fn test_network_mlkem_integration() {
    ...
    let hop_public_keys: Vec<_> = hop_exchanges.iter()
        .map(|kx| kx.public_key())
        .collect();
...
async fn test_network_dag_consensus_integration() {
    ...
    let node_configs: Vec<_> = (0..5).map(|i| NodeConfig {
        node_id: NodeId::new(format!("node_{}", i)),
        stake: 100,
        is_validator: true,
    }).collect();
    
    let nodes: Vec<_> = node_configs.into_iter()
...
async fn test_end_to_end_transaction_flow() {
    ...
    let quantum_keys: Vec<_> = routing_peers.iter()
    ...
    for (i, peer_id) in route.iter().enumerate() {
        ...
        if let Some(layer) = current_layers.pop() {
            ...
            if i == route.len() - 1 {
                ...
                let recovered_vertex: Vertex = bincode::deserialize(&next_layer.encrypted_data).unwrap();
                ...
                assert!(sender_keypair.public_key().verify(&tx_hash.as_bytes(), &signature));
...
async fn test_network_fingerprint_integration() {
    ...
    let authenticated_peers = peer_data.iter()
        .filter(|(peer_id, _, fingerprint)| {
            // In a real system, this would verify against known good fingerprints
            fingerprint.verify() && connection_manager.get_status(peer_id) == Some(ConnectionStatus::Connected)
        })
        .count();
...
async fn test_quantum_resistant_security_integration() {
    ...
    for security_level in [MlKemSecurityLevel::Level1, MlKemSecurityLevel::Level3, MlKemSecurityLevel::Level5] {
        ...
        let sensitive_data = format!("Quantum-resistant payload for security level {:?}", security_level).into_bytes();
...
async fn test_fault_tolerance_integration() {
    ...
    let nodes: Vec<_> = (0..total_nodes).map(|i| {
        let node_id = NodeId::new(format!("node_{}", i));
...
async fn test_performance_integration() {
    ...
    println!("Routing performance: {:.2} messages/second", messages_per_second);
    ...
    println!("Crypto performance: {:.2} operations/second", crypto_ops_per_second);
    ...
    assert!(e2e_time < Duration::from_millis(100), "End-to-end latency too high");
}