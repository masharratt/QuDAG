use qudag_network::{
    ...
    RoutingStrategy, Router, ConnectionStatus, SecureConnection, SecureConfig, TransportKeys
};
...
async fn test_malformed_message_handling() {
    ...
    let result = router.route(&invalid_msg, RoutingStrategy::Anonymous { hops: 2 }).await;
    ...
    let result = router.route(&empty_dest_msg, RoutingStrategy::Anonymous { hops: 2 }).await;
    ...
    let result = router.route(&oversized_id_msg, RoutingStrategy::Anonymous { hops: 2 }).await;
    assert!(result.is_ok()); // Should handle gracefully
}
...
async fn test_insufficient_peers_routing() {
    ...
    let result = router.route(&msg, RoutingStrategy::Anonymous { hops: 5 }).await;
...
async fn test_zero_length_messages() {
    ...
    let result = router.route(&empty_msg, RoutingStrategy::Direct(vec![1; 32])).await;
    assert!(result.is_ok());
}
...
async fn test_concurrent_routing() {
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            router_clone.route(&msg, RoutingStrategy::Anonymous { hops: 3 }).await
        });
        handles.push(handle);
    }
    ...
    assert!(success_count >= 10, "Too many routing failures: {}/20", success_count);
}
...
async fn test_network_partition_recovery() {
    ...
    for peer in peers.iter().take(2) {
        manager.update_status(*peer, ConnectionStatus::Failed("Network partition".into())).await;
        manager.disconnect(peer).await;
    }
    ...
    for peer in peers.iter().take(2) {
        ...
        manager.update_status(*peer, ConnectionStatus::Connected).await;
    }
...
async fn test_hop_info_consistency() {
    ...
    for (i, peer_id) in route.iter().enumerate() {
        ...
        assert!(info.can_decrypt_layer(i), "Peer {} cannot decrypt its own layer", i);
    }
}
...
async fn test_extreme_message_sizes() {
    ...
    let result = router.route(&large_msg, RoutingStrategy::Direct(vec![1; 32])).await;
    assert!(result.is_ok());
    ...
    let result = router.route(&max_msg, RoutingStrategy::Direct(vec![1; 32])).await;
...
async fn test_connection_status_transitions() {
    ...
    manager.update_status(peer_id, ConnectionStatus::Disconnecting).await;
    let status = manager.get_status(&peer_id).await;
    ...
    manager.connect(peer_id).await.unwrap();
    manager.update_status(peer_id, ConnectionStatus::Failed("Test failure".into())).await;
    let status = manager.get_status(&peer_id).await;
