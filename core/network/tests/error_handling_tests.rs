use qudag_network::{
    ConnectionManager, NetworkError, NetworkMessage, PeerId, MessagePriority, 
    RoutingStrategy, Router, ConnectionStatus, NetworkAddress
};
...
async fn test_network_error_types() {
    ...
    let result = router.route(&msg, RoutingStrategy::Direct(vec![1, 2, 3])).await;
...
async fn test_error_propagation() {
    ...
    manager.update_status(peer_id, ConnectionStatus::Failed("Network unreachable".into())).await;
    ...
    let result = router.route(&msg, RoutingStrategy::Direct(peer_id.to_bytes().to_vec())).await;
...
async fn test_error_recovery() {
    ...
    manager.update_status(peer_id, ConnectionStatus::Failed("Temporary network issue".into())).await;
...
async fn test_timeout_handling() {
    ...
    let result = router.route(&msg_with_short_ttl, RoutingStrategy::Anonymous { hops: 10 }).await;
...
async fn test_resource_exhaustion() {
    ...
    for _ in 0..2 {
        ...
        small_manager.update_status(peer_id, ConnectionStatus::Connected).await;
    }
...
async fn test_malformed_data_handling() {
    ...
    let result = router.route(&msg_with_long_id, RoutingStrategy::Flood).await;
    assert!(result.is_ok()); // Should handle gracefully
    ...
    let result = router.route(&msg_with_bad_peer, RoutingStrategy::Anonymous { hops: 1 }).await;
...
async fn test_network_address_errors() {
    ...
    // Test with IPv6
    let ipv6_addr = NetworkAddress::from_ip_port(
...
async fn test_error_message_quality() {
    ...
    let result = router.route(&msg, RoutingStrategy::Anonymous { hops: 10 }).await;
    ...
    let result = router.route(&msg, RoutingStrategy::Direct(vec![1, 2])).await;
...
async fn test_error_isolation() {
    ...
    manager1.update_status(peer1, ConnectionStatus::Connected).await;
    manager2.update_status(peer2, ConnectionStatus::Connected).await;
    
    // Cause error in manager1
    manager1.update_status(peer1, ConnectionStatus::Failed("Isolated error".into())).await;
...
async fn test_graceful_degradation_with_errors() {
    ...
    for (i, peer) in peers.iter().enumerate() {
        if i % 2 == 0 {
            manager.update_status(*peer, ConnectionStatus::Failed(format!("Failure {}", i))).await;
        }
    }
    
    // System should still be partially functional
    let remaining_connections = peers.iter()
        .filter(|peer| {
            futures::executor::block_on(async {
                matches!(manager.get_status(peer).await, Some(ConnectionStatus::Connected))
            })
        })
        .count();
    
    assert!(remaining_connections >= 2, "Too few connections remain: {}", remaining_connections);
