...
use qudag_network::{
    NatTraversalConfig, NatTraversalManager, NatType, PortMapping, PortMappingProtocol,
    StunClient, StunServer, TurnClient, TurnServer, HolePunchCoordinator,
    RelayManager, ConnectionManager, ConnectionUpgradeManager, NatInfo
};
...
async fn test_hole_punch_coordinator() {
    ...
    let result = coordinator.start_hole_punch(peer_id, local_candidates, remote_candidates).await;
...
async fn test_relay_manager() {
    ...
    match manager.establish_relay(peer_id).await {
        Ok(connection) => {
            ...
            assert!(connection.is_active.load(std::sync::atomic::Ordering::Relaxed));
        }
        Err(e) => {
...
async fn test_nat_traversal_manager_creation() {
    ...
    assert_eq!(stats.stun_success.load(std::sync::atomic::Ordering::Relaxed), 0);
    assert_eq!(stats.hole_punch_success.load(std::sync::atomic::Ordering::Relaxed), 0);
    assert_eq!(stats.relay_connections.load(std::sync::atomic::Ordering::Relaxed), 0);
}
...
async fn test_nat_types() {
    ...
    for nat_type in types {
        match nat_type {
            ...
            NatType::PortRestrictedCone => assert_eq!(format!("{:?}", nat_type), "PortRestrictedCone"),
...
async fn test_nat_traversal_integration() {
    ...
    match nat_manager.initialize().await {
        Ok(()) => {
            ...
            // Test statistics
            let stats = nat_manager.get_stats();
            println!("STUN attempts: {}", stats.stun_success.load(std::sync::atomic::Ordering::Relaxed) + 
                     stats.stun_failures.load(std::sync::atomic::Ordering::Relaxed));
        }
        Err(e) => {
            println!("NAT traversal initialization failed (expected in test): {}", e);
        }
    }
    
    // Test port mapping creation
    match nat_manager.create_port_mapping(8080, 8080, PortMappingProtocol::TCP).await {
        Ok(mapping) => {
            assert_eq!(mapping.local_port, 8080);
            assert_eq!(mapping.external_port, 8080);
            assert_eq!(mapping.protocol, PortMappingProtocol::TCP);
...
async fn test_concurrent_nat_operations() {
    ...
    let tasks = (0..5).map(|i| {
        ...
        tokio::spawn(async move {
            ...
            nat_manager.create_port_mapping(
                local_port,
                external_port,
    ...
    for result in results {
        match result {
            Ok(Ok(mapping)) => {
                println!("Port mapping created: {}:{}", mapping.local_port, mapping.external_port);
            }
            Ok(Err(e)) => {
