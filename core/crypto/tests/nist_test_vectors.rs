...
use qudag_crypto::{
    ...
    ml_kem::{MlKem768, Metrics as MlKemMetrics},
...
fn get_ml_kem_768_kat_vectors() -> Vec<MlKemKatVector> {
    vec![
        MlKemKatVector {
            count: 0,
            seed: [
                0x06, 0x1A, 0x06, 0x1C, 0x02, 0x83, 0x14, 0x0C, 0x23, 0x2F, 0x2F, 0x6B, 0x15, 0x1F, 0x25, 0x40,
                0x2E, 0x27, 0x19, 0x25, 0x32, 0x05, 0x16, 0x19, 0x1C, 0x3E, 0x3B, 0x14, 0x1F, 0x28, 0x27, 0x2D,
...
mod nist_test_vectors {
    ...
    fn test_ml_kem_768_kat_vectors() {
        for vector in &get_ml_kem_768_kat_vectors() {
            ...
            assert_eq!(pk.as_bytes().len(), 1184, "ML-KEM-768 public key size mismatch");
            assert_eq!(sk.as_bytes().len(), 2400, "ML-KEM-768 secret key size mismatch");
            
            // Test encapsulation with public key
            let (ct, ss) = MlKem768::encapsulate(&pk).unwrap();
            
            // Verify ciphertext and shared secret sizes
            assert_eq!(ct.as_bytes().len(), 1088, "ML-KEM-768 ciphertext size mismatch");
    ...
    fn test_ml_dsa_65_kat_vectors() {
        for vector in &get_ml_dsa_65_kat_vectors() {
            ...
            assert_eq!(keypair.secret_key().as_bytes().len(), 4032, 
            ...
            // Test verification
            assert!(MlDsa::verify(&vector.msg, &signature, keypair.public_key()).is_ok(),
            ...
            if !modified_msg.is_empty() {
                modified_msg[0] ^= 1;
                assert!(MlDsa::verify(&modified_msg, &signature, keypair.public_key()).is_err(),
            ...
            modified_sig[0] ^= 1;
            assert!(MlDsa::verify(&vector.msg, &modified_sig, keypair.public_key()).is_err(),
    ...
    fn test_cross_algorithm_independence() {
        ...
        assert_ne!(pk1.as_bytes(), pk3.as_bytes(), "ML-KEM keys should be independent");
        assert_ne!(sk1.as_bytes(), sk3.as_bytes(), "ML-KEM secret keys should be independent");
    ...
    fn test_deterministic_generation() {
        ...
        assert_ne!(pk1.as_bytes(), pk3.as_bytes(), "Different generations should produce different keys");
        assert_ne!(sk1.as_bytes(), sk3.as_bytes(), "Different generations should produce different secret keys");
    }
    ...
    fn test_algorithm_composition() {
        ...
        assert!(MlDsa::verify(&authenticated_message, &signature, dsa_keypair.public_key()).is_ok());
    ...
    fn test_interoperability_vectors() {
        ...
        for (description, message) in test_cases {
            ...
            assert!(MlDsa::verify(message, &signature, keypair.public_key()).is_ok(),
                "Verification failed for: {}", description);
        }
    }
