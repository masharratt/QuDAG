...
fn test_ml_dsa_key_generation() {
    ...
    // Check key sizes
    assert_eq!(keypair.public_key().len(), ml_dsa_65_params::PUBLIC_KEY_SIZE);
    assert_eq!(keypair.secret_key().len(), ml_dsa_65_params::SECRET_KEY_SIZE);
...
fn test_ml_dsa_verify_wrong_message() {
    ...
    assert!(matches!(result.unwrap_err(), MlDsaError::VerificationFailed));
}
...
fn test_ml_dsa_invalid_key_sizes() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), MlDsaError::InvalidSignatureLength));
}
...
fn test_ml_dsa_empty_message() {
    ...
    assert!(result.is_ok(), "Should be able to sign and verify empty message");
}
...
fn test_ml_dsa_large_message() {
    ...
    assert!(result.is_ok(), "Should be able to sign and verify large message");
}
...
fn test_ml_dsa_security_properties() {
    ...
    for window in secret_key_bytes.windows(16) {
        assert!(!signature.windows(16).any(|sig_window| sig_window == window),
                "Signature should not contain secret key data");
    }
...
fn test_ml_dsa_performance_benchmarks() {
    ...
    assert!(keygen_time < Duration::from_millis(100), 
            "Key generation too slow: {:?}", keygen_time);
    assert!(sign_time < Duration::from_millis(50), 
            "Signing too slow: {:?}", sign_time);
    assert!(verify_time < Duration::from_millis(50), 
            "Verification too slow: {:?}", verify_time);
