...
fn benchmark_throughput(c: &mut Criterion) {
    ...
    for duration in test_durations.iter() {
        group.bench_with_input(BenchmarkId::new("sign_throughput", duration), duration, |b, &duration| {
            b.iter_custom(|_| {
                let start = Instant::now();
                let mut count = 0;
                ...
                println!("Sign Throughput: {} ops/sec", count as f64 / duration as f64);
                std::time::Duration::from_secs(duration)
            });
        });
        
        group.bench_with_input(BenchmarkId::new("verify_throughput", duration), duration, |b, &duration| {
            b.iter_custom(|_| {
                let start = Instant::now();
                let mut count = 0;
                let (pk, sk) = MLDsa::keygen().expect("Key generation failed");
                ...
                println!("Verify Throughput: {} ops/sec", count as f64 / duration as f64);
                std::time::Duration::from_secs(duration)
            });
        });
    }
