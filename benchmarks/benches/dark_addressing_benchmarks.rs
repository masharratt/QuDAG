...
mod mock {
    ...
    impl DarkResolver {
        ...
        pub fn resolve_address(&self, domain: &str, _secret_key: &[u8]) -> Result<NetworkAddress, String> {
...
fn benchmark_shadow_address_routing(c: &mut Criterion) {
    ...
    let shadow_addresses: Vec<Vec<u8>> = (0..100)
        .map(|_| handler.generate_address())
        .collect();
...
fn benchmark_end_to_end_dark_routing(c: &mut Criterion) {
    ...
    dark_resolver.register_domain("test.dark", NetworkAddress::new([10, 0, 0, 1], 8080)).unwrap();
