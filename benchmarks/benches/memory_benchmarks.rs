...
impl MockNode {
    ...
    fn estimate_memory_usage(&self) -> usize {
        let connections_memory = self.connections.iter()
            .map(|c| c.buffer.len() + std::mem::size_of::<MockConnection>())
            .sum::<usize>();
        
        let vertices_memory = self.dag_vertices.iter()
            .map(|v| v.data.len() + v.references.len() * 8 + v.signature.len() + std::mem::size_of::<MockVertex>())
            .sum::<usize>();
        
        let keys_memory = self.crypto_keys.iter()
            .map(|k| k.public_key.len() + k.private_key.len() + std::mem::size_of::<MockKey>())
            .sum::<usize>();
        
        let messages_memory = self.message_queue.iter()
            .map(|m| m.payload.len() + m.routing_info.len() + std::mem::size_of::<MockMessage>())
            .sum::<usize>();
        
        connections_memory + vertices_memory + keys_memory + messages_memory + std::mem::size_of::<MockNode>()
    }
}
            ...
            |b, &conn_count| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("Connections: {}, Peak memory: {} MB, Estimated: {} MB", 
                                 conn_count, 
                                 peak_mem / (1024 * 1024),
                        ...
                        if peak_mem > 100 * 1024 * 1024 {
                            println!("WARNING: Peak memory usage {} MB exceeds 100MB target", 
                                     peak_mem / (1024 * 1024));
                        }
            ...
            |b, &vertex_count| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("Vertices: {}, Peak memory: {} MB, Estimated: {} MB", 
                                 vertex_count, 
                                 peak_mem / (1024 * 1024),
                                 estimated_mem / (1024 * 1024));
                ...
                |b, &(msg_count, msg_size)| {
                    b.iter_custom(|iters| {
                        ...
                        for _ in 0..iters {
                            ...
                            println!("Messages: {}x{} bytes, Peak memory: {} MB", 
                                     msg_count, msg_size, peak_mem / (1024 * 1024));
                            
                            // Calculate throughput
                            let total_data = msg_count * msg_size;
                            let throughput_mbps = (total_data as f64 / (1024.0 * 1024.0)) / duration.as_secs_f64();
            ...
            |b, &key_count| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        println!("Key sets: {}, Peak memory: {} MB", 
                                 key_count, peak_mem / (1024 * 1024));
                        
                        // Calculate memory per key set
                        let mem_per_key_set = peak_mem / key_count;
                        println!("Memory per key set (ML-KEM + ML-DSA + HQC): {} KB", 
    ...
    group.bench_function("realistic_node_workload", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                if peak_mem > 100 * 1024 * 1024 {
                    println!("ERROR: Node memory usage {} MB exceeds 100MB target!", 
                             peak_mem / (1024 * 1024));
                } else {
    ...
    group.bench_function("repeated_operations", |b| {
        b.iter_custom(|iters| {
            ...
            if memory_growth > 1024 * 1024 { // 1MB threshold
                println!("WARNING: Potential memory leak detected! Growth: {} MB", 
