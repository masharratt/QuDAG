use qudag_exchange_core::{FeeModel, AgentStatus, rUv, types::Timestamp, FeeCalculator};
...
fn test_fee_model_mathematical_requirements() {
    ...
    let rate1 = model.calculate_fee_rate(&agent1, Timestamp::new(0)).expect("Should calculate rate");
    println!("Test 1 - New unverified agent: {:.3}%", rate1 * 100.0);
    assert!((rate1 * 100.0 - 0.1).abs() < 0.01, "New agent should get ~0.1% fee");
    ...
    let rate2 = model.calculate_fee_rate(&agent2, Timestamp::new(three_months)).expect("Should calculate rate");
    println!("Test 2 - Medium usage unverified: {:.3}%", rate2 * 100.0);
    assert!((rate2 * 100.0 - 0.32).abs() < 0.1, "Medium usage agent should get ~0.32% fee");
    ...
    let rate3 = model.calculate_fee_rate(&agent3, Timestamp::new(six_months)).expect("Should calculate rate");
    println!("Test 3 - High usage unverified: {:.3}%", rate3 * 100.0);
    assert!(rate3 * 100.0 > 0.8 && rate3 * 100.0 <= 1.0, "High usage agent should approach 1.0%");
}
...
fn test_verified_agent_fees() {
    ...
    let rate4 = model.calculate_fee_rate(&agent4, Timestamp::new(0)).expect("Should calculate rate");
    println!("Test 4 - New verified agent: {:.3}%", rate4 * 100.0);
    assert!((rate4 * 100.0 - 0.25).abs() < 0.01, "New verified agent should get ~0.25% fee");
    ...
    let rate5 = model.calculate_fee_rate(&agent5, Timestamp::new(six_months)).expect("Should calculate rate");
    println!("Test 5 - High usage verified: {:.3}%", rate5 * 100.0);
    assert!((rate5 * 100.0 - 0.28).abs() < 0.1, "High usage verified agent should get ~0.28% fee");
}
...
fn test_verified_advantage() {
    ...
    let six_months = 6 * 30 * 24 * 60 * 60;
    let unverified_rate = model.calculate_fee_rate(&unverified, Timestamp::new(six_months)).expect("Should calculate rate");
    let verified_rate = model.calculate_fee_rate(&verified, Timestamp::new(six_months)).expect("Should calculate rate");
    
    println!("Verified advantage test - Unverified: {:.3}%, Verified: {:.3}%", 
             unverified_rate * 100.0, verified_rate * 100.0);
    assert!(verified_rate < unverified_rate, "Verified agent should pay lower fees than unverified with same usage");
}
...
fn test_fee_amount_calculation() {
    ...
    let fee_amount = model.calculate_fee_amount(transaction_amount, &agent, Timestamp::new(0))
        .expect("Should calculate fee amount");
    
    println!("Fee amount test - Transaction: {} rUv, Fee: {} rUv", 
             transaction_amount.amount(), fee_amount.amount());
    
    // Should be 1000 * 0.001 = 1 rUv (minimum fee rate)
    assert_eq!(fee_amount.amount(), 1, "Fee amount should be 1 rUv for 1000 rUv transaction at minimum rate");
}
...
fn test_edge_cases() {
    ...
    let rate = model.calculate_fee_rate(&agent, Timestamp::new(500)).expect("Should calculate rate");
    assert!(rate >= 0.0 && rate <= 1.0, "Fee rate should be in valid range even with edge case timing");
    
    // Test zero usage
    let agent_zero = AgentStatus::new_unverified(Timestamp::new(0));
    let rate_zero = model.calculate_fee_rate(&agent_zero, Timestamp::new(0)).expect("Should calculate rate");
    assert!((rate_zero - 0.001).abs() < 1e-10, "Zero usage should give minimum fee");
}
...
fn test_fee_calculator_examples() {
    ...
    assert!((examples[0].1 - 0.001).abs() < 1e-10, "First example should be minimum fee");
