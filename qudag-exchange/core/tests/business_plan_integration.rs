...
use qudag_exchange_core::{
    ExchangeConfig, ExchangeConfigBuilder, BusinessPlanConfig, PayoutConfig,
    ContributorRole, ContributorInfo, AccountId, rUv, FeeRouter,
    types::Timestamp,
};
...
fn test_fee_router_contributor_registration() {
    ...
    fee_router.register_contributor("agent_456".to_string(), contributor_info).unwrap();
...
fn test_fee_distribution_single_agent() {
    ...
    fee_router.register_contributor("agent_123".to_string(), contributor_info).unwrap();
...
fn test_fee_distribution_plugin_enhanced() {
    ...
    let agent_payout = payout_tx.payouts.iter()
        .find(|p| p.vault_id.as_str() == "vault_agent")
        .unwrap();
    let plugin_payout = payout_tx.payouts.iter()
