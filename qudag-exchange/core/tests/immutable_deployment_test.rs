use qudag_exchange_core::{
    ImmutableDeployment, LockableConfig, FeeModelParams, 
...
fn test_immutable_deployment_flow() -> Result<()> {
    ...
    assert!(!deployment.config.enabled, "Should start with immutable mode disabled");
    ...
    assert!(deployment.config.enabled, "Immutable mode should be enabled");
    ...
    assert!(deployment.config.is_in_grace_period(during_grace), "Should be in grace period");
    assert!(!deployment.config.is_in_grace_period(after_grace), "Should be past grace period");
    ...
    assert!(!deployment.config.is_enforced(during_grace), "Should not be enforced during grace");
    assert!(deployment.config.is_enforced(after_grace), "Should be enforced after grace");
...
fn test_configuration_validation() -> Result<()> {
    ...
    assert!(config.validate().is_err(), "Should reject zero max_total_supply");
...
fn test_immutable_config_state_transitions() -> Result<()> {
    ...
    assert!(config.disable().is_err(), "Should not allow disable when locked");
...
fn test_grace_period_edge_cases() -> Result<()> {
    ...
    let current_time = Timestamp::new(1000);
    assert!(!config.is_in_grace_period(current_time), "Should not be in grace period when not locked");
    
    // Test zero grace period
    config.locked_at = Some(Timestamp::new(1000));
    config.grace_period_seconds = 0;
    assert!(!config.is_in_grace_period(current_time), "Zero grace period should not be in grace period");
    ...
    assert!(!config.is_in_grace_period(boundary_time), "Should not be in grace at exact boundary");
    assert!(!config.is_in_grace_period(just_after), "Should not be in grace after boundary");
...
fn test_governance_key_management() -> Result<()> {
    ...
    assert!(config.set_governance_key(new_key).is_err(), "Should not allow governance key change when locked");
