...
use crate::{
    account::AccountId,
    types::{rUv, Timestamp, Nonce, Hash},
    Error, Result,
};
...
mod tests {
    ...
    fn test_transaction_creation() {
        ...
        match &tx.tx_type {
            TransactionType::Transfer { from: f, to: t, amount: a } => {
                assert_eq!(f, &from);
                assert_eq!(t, &to);
                assert_eq!(*a, amount);
            }
    ...
    fn test_transaction_builder() {
        ...
        let tx = TransactionBuilder::new()
            .with_type(TransactionType::Mint { to: to.clone(), amount })
            .with_nonce(nonce)
            .with_fee(fee)
            .with_memo("Test mint")
            .with_chain_id(42)
