...
impl ConsensusAdapter {
    ...
    pub fn get_transaction_status(&self, tx_id: &TransactionId) -> Option<TransactionStatus> {
        ...
        #[cfg(feature = "std")]
        return self.pending_transactions.get(tx_id).map(|entry| entry.status);
    }
    ...
    pub fn get_confidence(&self, tx_id: &TransactionId) -> Option<f64> {
        ...
        #[cfg(feature = "std")]
        return self.pending_transactions.get(tx_id).map(|entry| entry.confidence);
    }
    ...
    pub fn simulate_consensus_round(&mut self) -> Vec<ConsensusEvent> {
        ...
        let tx_ids: Vec<_> = {
            ...
            #[cfg(feature = "std")]
            let ids = self.pending_transactions.iter()
                .map(|entry| *entry.key())
                .collect();
    ...
    fn compute_vertex_id(
        ...
        Ok(VertexId::from_hash(Hash::from_bytes(*hash_bytes.as_bytes())))
    }
    
    /// Get all pending transactions
    pub fn pending_transactions(&self) -> Vec<(TransactionId, PendingTransaction)> {
        #[cfg(not(feature = "std"))]
        return self.pending_transactions.iter()
            .map(|(k, v)| (*k, v.clone()))
            .collect();
