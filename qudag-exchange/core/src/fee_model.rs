...
use crate::{types::{rUv, Timestamp}, Error, Result};
...
impl FeeModelParams {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.f_max <= self.f_min || self.f_max >= 1.0 {
            return Err(Error::Other("f_max must be greater than f_min and less than 1".into()));
        }
        if self.f_min_verified <= 0.0 || self.f_min_verified >= 1.0 {
            return Err(Error::Other("f_min_verified must be between 0 and 1".into()));
        }
        if self.f_max_verified <= self.f_min_verified || self.f_max_verified >= 1.0 {
            return Err(Error::Other("f_max_verified must be greater than f_min_verified and less than 1".into()));
        }
        if self.time_constant_seconds == 0 {
            return Err(Error::Other("time_constant_seconds must be greater than 0".into()));
        }
        if self.usage_threshold_ruv == 0 {
            return Err(Error::Other("usage_threshold_ruv must be greater than 0".into()));
        }
        Ok(())
    }
}
...
impl FeeModel {
    ...
    pub fn calculate_fee_rate(
        ...
        let time_since_first = if current_time.value() >= agent_status.first_transaction_timestamp.value() {
...
impl FeeCalculator {
    ...
    pub fn calculate_examples() -> Vec<(String, f64)> {
        ...
        let rate1 = model.calculate_fee_rate(&status1, Timestamp::new(0)).unwrap();
        ...
        let rate2 = model.calculate_fee_rate(&status2, Timestamp::new(three_months)).unwrap();
        ...
        let rate3 = model.calculate_fee_rate(&status3, Timestamp::new(six_months)).unwrap();
...
mod tests {
    ...
    fn test_fee_calculation_edge_cases() {
        ...
        let rate = model.calculate_fee_rate(&agent, Timestamp::new(1000)).unwrap();
        assert!((rate - 0.001).abs() < 1e-10); // Should be f_min
        
        // Test with time before first transaction (edge case)
        let rate = model.calculate_fee_rate(&agent, Timestamp::new(500)).unwrap();
    ...
    fn test_fee_amount_calculation() {
        ...
        let fee_amount = model.calculate_fee_amount(
            transaction_amount, 
            &agent, 
            Timestamp::new(0)
        ).unwrap();
    ...
    fn test_verified_vs_unverified_fees() {
        ...
        println!("Unverified rate: {}, Verified rate: {}", unverified_rate, verified_rate);
