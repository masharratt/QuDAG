...
impl ImmutableConfig {
    ...
    pub fn disable(&mut self) -> Result<()> {
        if self.is_locked() {
            return Err(Error::Other("Cannot disable immutable mode: system is locked".into()));
        }
        self.enabled = false;
        Ok(())
    }
    ...
    pub fn set_grace_period(&mut self, seconds: u64) -> Result<()> {
        if self.is_locked() {
            return Err(Error::Other("Cannot change grace period: system is locked".into()));
        }
    ...
    pub fn set_governance_key(&mut self, key: Vec<u8>) -> Result<()> {
        if self.is_locked() {
            return Err(Error::Other("Cannot set governance key: system is locked".into()));
        }
...
impl LockableConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_total_supply == 0 {
            return Err(Error::Other("max_total_supply must be greater than 0".into()));
        }
        ...
        if self.protocol_version == 0 {
            return Err(Error::Other("protocol_version must be greater than 0".into()));
        }
...
impl ImmutableDeployment {
    ...
    pub fn verify_lock_signature(&self, current_time: Timestamp) -> Result<bool> {
        let sig_data = self.config.lock_signature.as_ref()
            .ok_or_else(|| Error::Other("No lock signature present".into()))?;
        
        let locked_at = self.config.locked_at
    ...
    pub fn update_fee_params(&mut self, params: FeeModelParams, current_time: Timestamp) -> Result<()> {
        if !self.can_modify_config(current_time) {
            return Err(Error::Other("Cannot modify configuration: system is immutably locked".into()));
        }
    ...
    pub fn governance_override(
        ...
        let governance_key = self.config.governance_key.as_ref()
            .ok_or_else(|| Error::Other("No governance key set".into()))?;
        
        // Verify governance key matches
        let public_key = governance_keypair.to_public_key()
...
mod tests {
    ...
    fn test_immutable_deployment_lifecycle() {
        ...
        deployment.update_fee_params(new_params, current_time).unwrap();
