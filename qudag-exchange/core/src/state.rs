...
impl LedgerState {
    ...
    fn calculate_transaction_cost(&self, transaction: &Transaction) -> Result<rUv> {
        ...
        let units = ResourceMeter::estimate_transfer_units(
            transaction.metadata.memo.as_deref()
        );
    ...
    fn add_transaction_record(&mut self, record: TransactionRecord) {
        ...
        if self.recent_transactions.len() > max_history {
            self.recent_transactions.drain(0..self.recent_transactions.len() - max_history);
        }
    }
    ...
    pub fn create_checkpoint(&mut self) -> Result<StateCheckpoint> {
        ...
        let checkpoint = StateCheckpoint {
            ...
            state_root: self.metadata.state_root
...
impl StateManager {
    ...
    pub fn load_state(path: &str) -> Result<LedgerState> {
        ...
        let bytes = fs::read(path)
            .map_err(|e| Error::Other(format!("Failed to load state: {}", e)))?;
...
mod tests {
    ...
    fn test_apply_transaction() {
        ...
        // Create accounts
        state.ledger.create_account(AccountId::new("alice")).unwrap();
        state.ledger.create_account(AccountId::new("bob")).unwrap();
        state.ledger.mint(&AccountId::new("alice"), rUv::new(1000)).unwrap();
        ...
        assert_eq!(state.ledger.get_balance(&AccountId::new("alice")).unwrap(), rUv::new(895)); // 1000 - 100 - 5
        assert_eq!(state.ledger.get_balance(&AccountId::new("bob")).unwrap(), rUv::new(100));
