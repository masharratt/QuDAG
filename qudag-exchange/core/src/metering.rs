...
impl ResourceMeter {
    ...
    fn set_default_costs(&mut self) {
        ...
        self.cost_table.insert(OperationType::Transfer, OperationCost {
            base_cost: rUv::new(1),
        ...
        self.cost_table.insert(OperationType::Mint, OperationCost {
        ...
        // Burn: 5 rUv base
        self.cost_table.insert(OperationType::Burn, OperationCost {
            base_cost: rUv::new(5),
            per_unit_cost: rUv::ZERO,
            min_fee: rUv::new(5),
            max_units: 0,
        ...
        self.cost_table.insert(OperationType::CreateAccount, OperationCost {
        ...
        self.cost_table.insert(OperationType::UpdateAccount, OperationCost {
            base_cost: rUv::new(5),
            per_unit_cost: rUv::ZERO,
            min_fee: rUv::new(5),
            max_units: 0,
        ...
        self.cost_table.insert(OperationType::StoreData, OperationCost {
            base_cost: rUv::new(1),
        ...
        self.cost_table.insert(OperationType::ExecuteComputation, OperationCost {
            base_cost: rUv::new(1),
        ...
        self.cost_table.insert(OperationType::VerifySignature, OperationCost {
            base_cost: rUv::new(2),
            per_unit_cost: rUv::ZERO,
            min_fee: rUv::new(2),
            max_units: 0,
        ...
        self.cost_table.insert(OperationType::QueryBalance, OperationCost {
        ...
        self.cost_table.insert(OperationType::QueryTransaction, OperationCost {
            base_cost: rUv::new(0),
            per_unit_cost: rUv::ZERO,
            min_fee: rUv::new(1),
            max_units: 0,
    ...
    pub fn calculate_cost(
        ...
        let cost_config = self.cost_table.get(&operation)
        ...
        // Calculate base cost
        let unit_cost = cost_config.per_unit_cost
            .checked_mul(units)
            .ok_or_else(|| Error::Other("Cost calculation overflow".into()))?;
        
        let raw_cost = cost_config.base_cost
    ...
    fn record_usage(&mut self, operation: OperationType, cost: rUv) {
        ...
        *self.usage_stats.operations_by_type.entry(operation).or_insert(0) += 1;
        
        let current = self.usage_stats.ruv_by_type.entry(operation)
            .or_insert(rUv::ZERO);
...
mod tests {
    ...
    fn test_cost_calculation() {
        ...
        let cost = meter.calculate_cost(OperationType::ExecuteComputation, 5).unwrap();
    ...
    fn test_transfer_size_estimation() {
        ...
        assert_eq!(ResourceMeter::estimate_transfer_units(Some(&large_memo)), 2); // 200 + 900 > 1KB
    }
}