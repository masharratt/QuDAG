...
impl FeeRouter {
        ...
        if let Some(pct) = info.custom_percentage {
            if pct < 0.0 || pct > self.config.max_contributor_percentage {
                return Err(Error::Other(
                    format!("Custom percentage {} exceeds maximum allowed {}", 
                            pct, self.config.max_contributor_percentage).into()
                ));
            }
        }
    ...
    pub fn distribute_fees(
        ...
        for role in contributor_roles {
            ...
            if let Some(contributor_info) = self.contributors.get(&contributor_id) {
                ...
                total_distributed = total_distributed.add(payout_amount)
        ...
        let system_fee = total_fee.multiply(self.config.system_fee_percentage)
            .map_err(|e| Error::Other(e.into()))?;
        total_distributed = total_distributed.add(system_fee)
            .map_err(|e| Error::Other(e.into()))?;
    ...
    pub fn process_pending_payouts(&mut self, current_time: Timestamp) -> Result<Vec<PayoutTransaction>> {
    ...
    fn determine_split_template(&self, roles: &[ContributorRole]) -> Result<String> {
        let has_plugin = roles.iter().any(|r| matches!(r, ContributorRole::PluginCreator { .. }));
        let has_node = roles.iter().any(|r| matches!(r, ContributorRole::NodeOperator { .. }));
        let has_bounty = roles.iter().any(|r| matches!(r, ContributorRole::BountyAgent { .. }));
    ...
    fn calculate_payout(
        ...
        let payout_amount = total_fee.multiply(final_percentage)
            .map_err(|e| Error::Other(e.into()))?;
    ...
    fn get_base_percentage_for_role(
        ...
        let template = match split_template {
            ...
            _ => return Err(Error::Other(format!("Unknown split template: {}", split_template).into())),
        };
        ...
        template.percentages.iter()
            .find(|(ct, _)| ct == &contributor_type)
            .map(|(_, pct)| *pct)
            .ok_or_else(|| Error::Other(
                format!("No percentage found for contributor type: {:?}", contributor_type).into()
            ))
    }
    ...
    fn update_contributor_earnings(
        ...
        if let Some(contributor) = self.contributors.get_mut(contributor_id) {
            contributor.total_earnings = contributor.total_earnings.add(amount)
                .map_err(|e| Error::Other(e.into()))?;
    ...
    fn cleanup_old_history(&mut self, current_time: Timestamp) {
        let cutoff_time = current_time.value().saturating_sub(self.config.audit_retention_seconds);
        self.payout_history.retain(|tx| tx.distributed_at.value() >= cutoff_time);
    }
}
...
impl PayoutConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.max_contributor_percentage <= 0.0 || self.max_contributor_percentage > 1.0 {
            return Err(Error::Other("max_contributor_percentage must be between 0.0 and 1.0".into()));
        }
        
        if self.system_fee_percentage < 0.0 || self.system_fee_percentage > 0.1 {
            return Err(Error::Other("system_fee_percentage must be between 0.0 and 0.1 (10% max)".into()));
        }
        
        if self.min_payout_threshold.amount() == 0 {
            return Err(Error::Other("min_payout_threshold must be greater than 0".into()));
        }
        ...
        for (name, split) in [
            ...
            if total > 1.0 {
                return Err(Error::Other(
                    format!("Split template '{}' percentages sum to {}, must be <= 1.0", name, total).into()
                ));
            }
        }
...
mod tests {
    ...
    fn test_contributor_registration() {
        ...
        router.register_contributor("agent-1".to_string(), contributor_info).unwrap();
    ...
    fn test_payout_distribution() {
        ...
        let payout_tx = router.distribute_fees(
            "tx-1".to_string(),
            rUv::new(1000),
            roles,
            Timestamp::new(2000),
