...
impl MockLedger {
    ...
    fn transfer(&self, from: &AccountId, to: &AccountId, amount: Balance) -> Result<(), &'static str> {
        ...
        *from_balance -= amount;
        ...
        self.balances.entry(*to).and_modify(|b| *b += amount).or_insert(amount);
        Ok(())
    }
}
    ...
    for size in [1000, 10_000, 100_000, 1_000_000] {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                ...
                b.iter(|| {
                    ledger.transfer(black_box(&from), black_box(&to), black_box(1))
                });
            }
        );
    }
